...
public final class OntimizeJpaUtils {
     ...
     *            the data
     * @param columnList
     *            the column list; if its null, creates an EntityResult with ColumnNames from bean
     * @return the entity result
     * @throws Exception
     *             the exception
     */
    public static EntityResult transformListToEntityResultBeans(final List<?> data, List<String> columnList) throws Exception {
     ...
     *
     * @param attribute
     *            The name of field is going to query.
     * @param aBean
     *            The instance of a bean is going to query.
    ...
    protected static Object getAttributeBeanLast(final String attribute, final Object aBean) throws Exception {
        return OntimizeJpaUtils.changeDataTypeGet(JPAUtils.invokeGetValueDeclaredMethod(JPAUtils.getDeclaredGetMethod(attribute, aBean), aBean));
    }
     ...
     *
     * @param object
     *            Some value to set some attribute of a bean.
    ...
    public static Hashtable<String, Integer> getSQLTypes(final Query selectQuery, final List<String> validAttributes, final Class<?> anEntityClass, final EntityManager em) throws Exception {
        ...
        for (final String vat : validAttributes) {
            ...
            try {
                ...
                // FIXME meter log
				OntimizeJpaUtils.logger.warn("Unable to locate SingularAttribute in OntimizeJPAUtils.getSQLTypes");
     ...
     *             the exception
     */
    public static Hashtable<String, Integer> getSQLTypes(final Query selectQuery, final List<String> validAttributes, final Class<?> anEntityClass) throws Exception {
            ...
            if ((map != null) && (map.size() > 0)) {
                for (Entry<String, Object> entry : map.entrySet()) {
                    try {
                        ...
                        Method meth = BeanUtils.findDeclaredMethodWithMinimalParameters(value.getClass(), MappingInfoUtils.buildGetterMethodName(attToCopy, null));
                        if (meth == null) {
                            OntimizeJpaUtils.logger.error("Key '" + attToCopy + "' does not have a getter method for embedded attributed declared on bean " + value.getClass());
                        } else {
    ...
    /**
     * Reorganize attributes in beans if necessary. It checks if there are attributes in attributesValues that should load an entity into a relation of the current entity.
     *
    ...
    /**
     * Load entity relations present in attributes. It loads 'entity' relation using 'organizedRelatedBeansAttributes' information
     *
     * @param entityManager
     *            the entity manager
     * @param entity
     *            the entity
     * @param organizedRelatedBeansAttributes
    ...
    /**
     * To bean entity relation. It checks if 'entity' has a relation attribute bean named 'relationAttName', if so, it tries to load its bean using 'relationAttValues'
     *
     * @param entityManager
     *            the entity manager
     * @param entity
     ...
     *            the relationAttName
     * @param value
     *            the relationAttValues
     * @return the object bean or null it 'entity' doesn't have a relation named 'relationAttName'
     */
    private static Object toBeanEntityRelation(EntityManager entityManager, ManagedType<?> entity, String relationAttName, Map<String, Object> relationAttValues) {
        Attribute<?, ?> declaredAttribute = entity.getDeclaredAttribute(relationAttName);
        if ((declaredAttribute == null) && (entity instanceof IdentifiableType)) { // search in super types it it contains an attribute named 'relationAttName'
                    ...
                    if (sattr.getPersistentAttributeType().equals(PersistentAttributeType.BASIC)
                        ...
                        try {
                            ...
                            if (parameterTypes.length != 1) {
                                ...
                                final Object converted = DataConversorsUtil.convert(reorganizedAttributes.get(sattr.getName()), parameterTypes[0]);
    ...
    /**
     * Load bean. try loading a reference to a bean using its primary key, but if it it not able to identify its primary key in relationAttValues then it does a query
     *
     * TODO: MIRAR LA POSIBILIDAD DE POSTERGAR LA CARGA/BUSQUEDA DEL BEAN HASTA EL MOMENTO DE LA INSERCION DEVOLVIENDO UN PROXY ASI NO TENEMOS PORQUE TENER AQUI EL entityManager
     *
     * @param entityManager
     *            the entity manager
     * @param javaType
     *            the java type
     * @param relationAttValues
     *            the relation att values
     * @return the object
     */
    private static Object loadEntityBean(EntityManager entityManager, Class<?> javaType, Map<String, Object> relationAttValues) {
...
//                return realKeys.get(idClassAttribute.getName());
                ...
                for (SingularAttribute<? super Object, ?> sattr : idClassAttributes) {
                    ...
                    try {
                        ...
                        if (parameterTypes.length != 1) {
                            ...
                            final Object converted = DataConversorsUtil.convert(realKeys.get(sattr.getName()), parameterTypes[0]);
    ...
    /**
     * Organize dotted attributes into hierarchy. Those attributes not organized are returned inside 'remaining' parameter
     *
     * @param attributesValues
     *            the attributes values
     * @param remaining
    ...
    private static Map<String, Map<String, Object>> organizeDottedAttributesIntoHierarchy(Map<String, ?> attributesValues, Map<String, Object> remaining) {
        ...
        for (Entry<String, ?> entry : attributesValues.entrySet()) {
            ...
            if ((dotIdx > 0) && (dotIdx < (key.length() - 1))) {
                ...
                // they sure are not bean attributes that need to be created (it is possible that in value there is an already created attribute)
