...
public class OntimizePersistenceUnitManager extends DefaultPersistenceUnitManager {
    ...
    private final List<String> alreadyObtained = new ArrayList<>();
    ...
    private Map<String, PersistenceUnitInfo> getPersistenceUnitInfos() {
        try {
            Field f = this.getClass()
                .getSuperclass()
                .getField(OntimizePersistenceUnitManager.PERSISTENCE_UNIT_INFOS_PARENT_CLASS_FIELD_NAME);
    ...
    /**
     * Gets the persistence unit info namess using reflection because our parent didn't granted us
     * access
     * @return the persistence unit infos
     */
    private Set<String> getPersistenceUnitInfoNames() {
        try {
            Field f = this.getClass()
                .getSuperclass()
                .getField(OntimizePersistenceUnitManager.PERSISTENCE_UNIT_INFO_NAMES_PARENT_CLASS_FIELD_NAME);
            f.setAccessible(true);
    ...
    public PersistenceUnitInfo obtainPersistenceUnitInfo(String persistenceUnitName) {
        ...
        List<SpringPersistenceUnitInfo> persistenceUnitInfosToMerge = this
            .getPersistenceUnitInfosToMerge(persistenceUnitName);
        ...
        if (spui == null) {
            ...
            if (spui.isPossibleChild() && (!spui.isExcludeUnlistedClassesSetted() || !spui.isJtaDataSourceSetted()
                    || !spui.isNonJtaDataSourceSetted() || !spui
                        .isPersistenceProviderClassNameSetted()
                    || !spui.isSharedCacheModeSetted() || !spui.isTransactionTypeSetted()
                    || !spui.isValidationModeSetted())) {
                throw new IllegalStateException(
                        "Persistence unit with name '" + persistenceUnitName
                                + "' seems to be a child persistence unit but it (or its parent) hasn't defined all mandatory attributes for child persistence units");
            }
        }
    ...
    private SpringPersistenceUnitInfo mergePersistenceUnitInfos(
            List<SpringPersistenceUnitInfo> persistenceUnitInfosToMerge, String persistenceUnitName) {
        SpringPersistenceUnitInfo spui = null;
        if ((persistenceUnitInfosToMerge != null) && !persistenceUnitInfosToMerge.isEmpty()) {
            ...
            if (persistenceUnitInfosToMerge.size() > 1) {
                SpringPersistenceUnitInfo parentPersistenceUnitInfo = this.mergePersistenceUnitInfos(
                        persistenceUnitInfosToMerge.subList(1, persistenceUnitInfosToMerge.size()),
                        persistenceUnitInfosToMerge.get(1).getPersistenceUnitName());
    ...
    private SpringPersistenceUnitInfo merge(SpringPersistenceUnitInfo original, SpringPersistenceUnitInfo parent) {
        ...
        if ((original.getPersistenceProviderPackageName() == null)
                && (parent.getPersistenceProviderPackageName() != null)) {
    ...
    private List<SpringPersistenceUnitInfo> getPersistenceUnitInfosToMerge(String persistenceUnitName) {
        Map<String, PersistenceUnitInfo> persistenceUnitInfos = this.getPersistenceUnitInfos();
        List<SpringPersistenceUnitInfo> spis = new ArrayList<>();
        ...
        if (persistenceUnitInfo != null) {
            ...
            for (Entry<String, PersistenceUnitInfo> entry : persistenceUnitInfos.entrySet()) {
                ...
                if (currentPU.startsWith(persistenceUnitName)) {
                    ...
                    if (currentPU.substring(persistenceUnitName.length())
                        .startsWith(OntimizePersistenceUnitManager.EXTENDS_SEPARATOR)) {
                        String extendedPU = StringUtils.substringAfter(currentPU,
                                OntimizePersistenceUnitManager.EXTENDS_SEPARATOR);
    ...
    private SpringPersistenceUnitInfo toSpringPersistenceUnitInfo(PersistenceUnitInfo persistenceUnitInfo) {
        ...
        if (persistenceUnitInfo instanceof SpringPersistenceUnitInfo) {
            ...
            OntimizePersistenceUnitManager.logger
                .warn("trying to convert from " + persistenceUnitInfo.getClass().getCanonicalName() + " to logger");
