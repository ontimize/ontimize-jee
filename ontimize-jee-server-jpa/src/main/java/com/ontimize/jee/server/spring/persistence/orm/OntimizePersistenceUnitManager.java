...
public class OntimizePersistenceUnitManager extends DefaultPersistenceUnitManager {
	...
	public PersistenceUnitInfo obtainPersistenceUnitInfo(String persistenceUnitName) {
		...
		if (spui == null) {
			...
			if (spui.isPossibleChild() && (!spui.isExcludeUnlistedClassesSetted() || !spui.isJtaDataSourceSetted() || !spui.isNonJtaDataSourceSetted() || !spui
			        .isPersistenceProviderClassNameSetted() || !spui.isSharedCacheModeSetted() || !spui.isTransactionTypeSetted() || !spui.isValidationModeSetted())) {
	...
	private SpringPersistenceUnitInfo mergePersistenceUnitInfos(List<SpringPersistenceUnitInfo> persistenceUnitInfosToMerge, String persistenceUnitName) {
		...
		if ((persistenceUnitInfosToMerge != null) && !persistenceUnitInfosToMerge.isEmpty()) {
			...
			if (persistenceUnitInfosToMerge.size() > 1) {
				SpringPersistenceUnitInfo parentPersistenceUnitInfo = this.mergePersistenceUnitInfos(persistenceUnitInfosToMerge.subList(1, persistenceUnitInfosToMerge.size()),
