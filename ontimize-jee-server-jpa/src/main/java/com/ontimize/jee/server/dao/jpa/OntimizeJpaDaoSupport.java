...
public class OntimizeJpaDaoSupport implements ApplicationContextAware, IOntimizeDaoSupport {
    ...
    protected <T> T innerQuery(final Map<?, ?> keysValues, final List<String> validAttributes, final List<?> sort, final String queryId, Class<T> resultStyleClass, Class<?> queryResultClass, boolean failIfTemplateInfoNotMatch) throws SQLException, Exception {
    ...
    protected <T> List<T> executeQuery(final Query selectQuery, final List<String> columnNames, final QueryTemplateInformation queryTemplateInformation) throws SQLException {
        ...
        final List<T> translated = new ArrayList<T>();
    ...
    public EntityResult update(final Map<?, ?> attributesValues, final Map<?, ?> keysValues) {
        ...
        final Map<Object, Object> kvValidKeysValues = new HashMap<Object, Object>();
        ...
        if (entity != null) { // entity exists try JPQL update
            ...
            if (allInkeys) {
                ...
                if ((totalData != null) && (totalData.size() == 1)) {
                    ...
                    for (final Entry<String, ?> e : attibutesValuesBeansAsociationsConverted.entrySet()) {
                        ...
                        try {
                            ...
                            if ((attribute != null) && attribute.getPersistentAttributeType().equals(PersistentAttributeType.EMBEDDED)) {
                                ...
                                if (meth2 == null) {
                                    OntimizeJpaDaoSupport.logger.warn("Key '" + e.getKey() + "' does not have a getter method for embedded attributed declared on bean " + cl.getName());
                                } else {
                    ...
                    return new EntityResult(EntityResult.OPERATION_WRONG, EntityResult.OPERATION_WRONG, "something happened persisting changes in entity " + cl.getCanonicalName() + ", more than one result returned with primary keys");
                } else {
                    OntimizeJpaDaoSupport.logger.warn("nothing to update");
                    result.setCode(EntityResult.OPERATION_SUCCESSFUL_SHOW_MESSAGE);
                    result.setMessage(I18NNaming.M_IT_HAS_NOT_CHANGED_ANY_RECORD);
    ...
    public EntityResult delete(final Map<?, ?> keysValues) {
        ...
        // try safe update going through primary key
        final Map<?, ?> kvWithoutReferenceAttributes = this.processReferenceDataFieldAttributes(keysValues);
        final Hashtable<Object, Object> kvValidKeysValues = new Hashtable<Object, Object>();
        final Map<?, ?> processMultipleValueAttributes = this.processMultipleValueAttributes(kvWithoutReferenceAttributes);
        if (processMultipleValueAttributes != null) {
            kvValidKeysValues.putAll(processMultipleValueAttributes);
        }
        ...
        if (entity != null) { // entity exists try JPQL update
            ...
            if (allInkeys && !keys.isEmpty()) {
                ...
                if ((totalData != null) && (totalData.size() == 1)) {
                    ...
                    return new EntityResult(EntityResult.OPERATION_WRONG, EntityResult.OPERATION_WRONG, "something happened deleting entity " + cl.getCanonicalName() + ", more than one result returned with primary keys");
                } else {
    ...
    public EntityResult unsafeUpdate(final Map<?, ?> attributesValues, final Map<?, ?> keysValues) {
        ...
        if (entity != null) { // entity exists try JPQL update
            final Set<String> bck = new HashSet<String>(validAttributes.keySet());
            for (Iterator<String> keys = bck.iterator(); keys.hasNext();) {
                ...
                try {
                    ...
                    return new EntityResult(EntityResult.OPERATION_WRONG, EntityResult.OPERATION_WRONG, "entity " + cl.getCanonicalName() + " has not unique method for attribute " + key);
                }
     ...
     * @return the string
     */
    protected String prepareQuery(final Syntax syntax, final QueryTemplateInformation queryTemplateInformation, final List<String> attributes, final Map<?, ?> keysValues, final List<Object> sort, final Class<?> beanClass) {

        List<String> vattributes = this.processReferenceDataFieldAttributes(attributes);

        final Map<?, ?> kvWithoutReferenceAttributes = this.processReferenceDataFieldAttributes(keysValues);
        Map<Object, Object> kvValidKeysValues = new HashMap<Object, Object>();
        final Map<?, ?> processMultipleValueAttributes = this.processMultipleValueAttributes(kvWithoutReferenceAttributes);
        if (processMultipleValueAttributes != null) {
            kvValidKeysValues.putAll(processMultipleValueAttributes);
        }
        ...
        if ((queryTemplateInformation == null) && syntax.equals(Syntax.JPQL)) {
            ...
            if (vattributes != null) {
                ...
                vattributes = new ArrayList<String>();
            }
        ...
        for (String whereKey : whereKeys) {
            ...
            template = template.replaceAll(whereKey,
    ...
    private String createWhereCondition(String whereKey, final Syntax syntax, final Map<Object, Object> kvValidKeysValues, String beanPrefix) {
        ...
        if (((whereKey == null) || whereKey.equals(OntimizeJpaDaoSupport.PLACEHOLDER_WHERE)) && (kvValidKeysValues != null)) {
            Map<Object, Object> map = new HashMap<Object, Object>(kvValidKeysValues);
     ...
     * <p>
     * Returns a hashtable containing all the objects contained in the argument <code>keysValues</code> except in the case of keys that are ReferenceFieldAttribute objects, which are replaced by ((ReferenceFieldAttribute)object).getAttr()
     * <p>
     *
     * @param keysValues
     ...
     * <p>
     * Returns a List containing all the objects in the argument <code>list</code> except in the case of keys that are ReferenceFieldAttribute objects, which are maintained but also ((ReferenceFieldAttribute)object).getAttr() is added
     * <p>
     *
     * @param list
     *            the list to process
     ...
     * @return the string
     */
    protected String transformColumnName(String ob, final MappingInfo mappingInfo, final List<AmbiguousColumnType> ambiguousColumns, final List<FunctionColumnType> functionColumns) {
        if (mappingInfo != null) {
    ...
    /**
     * Processes the MultipleValue objects contained in <code>keysValues</code>. Returns a new Hashtable with the same
     * data as <code>keysValues</code> except that MultipleValue objects are deleted and the key-value pairs of these
     * objects are added to the new Hashtable.
     *
     * @param keysValues
     *            the keys values
    ...
    private List<String> adaptAttributes(final List<?> attributes) {
        final List<String> validAttr = new ArrayList<String>();
        if (attributes != null) {
    ...
    private List<Object> adaptSort(final List<?> sort) {
        final List<Object> validSort = new ArrayList<Object>();
        if (sort != null) {
