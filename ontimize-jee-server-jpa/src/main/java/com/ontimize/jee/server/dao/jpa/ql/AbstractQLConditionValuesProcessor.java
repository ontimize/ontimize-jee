...
public abstract class AbstractQLConditionValuesProcessor implements QLConditionValuesProcessor {
    ...
    /**
     * Creates a <code>AbstractQLConditionValuesProcessor</code> where every condition that uses
        ...
        while (enumKeys.hasNext()) {
            ...
            if (oKey instanceof String) {
                ...
                if (oValue instanceof String) {
                    sbStringQuery = this.appendStringComparisonClause(sbStringQuery, (String) oKey, (String) oValue,
                            isLast, encloseColumnName,
                            (wildcards == null) || wildcards.isEmpty() || wildcards.contains(oKey));
                } else if (oValue instanceof SearchValue) {
                    sbStringQuery = this.addSearchValue(sbStringQuery, (String) oKey, (SearchValue) oValue, isLast,
                            encloseColumnName);
                } else if (oValue instanceof NoWildCard) {
                    sbStringQuery = this.appendStringComparisonClause(sbStringQuery, (String) oKey,
                            ((NoWildCard) oValue).getValue(), isLast, encloseColumnName, false);
                } else {
    ...
    protected StringBuilder appendStringComparisonClause(final StringBuilder sbStringQuery, final String oKey,
            final String oValue, final boolean isLast,
        ...
        if (this.valueToQLProcessor != null) {
            targetValue = this.valueToQLProcessor.toQLLiteral(oValue);
        }
        ...
        if (wildcarded && ((targetValue.indexOf(SQLStatementBuilder.ASTERISK_CHAR) >= 0)
                || (targetValue.indexOf(SQLStatementBuilder.INTERROG) >= 0))) {
    ...
    protected StringBuilder addSearchValue(final StringBuilder sbStringQuery, final String oKey,
            final SearchValue oValue, final boolean isLast, final boolean encloseColumnName) {
        StringBuilder sbStringQueryInner = sbStringQuery;
        if (oValue == null) {
            return sbStringQueryInner;
        ...
        } else if ((oValue.getCondition() == SearchValue.BETWEEN)
                || (oValue.getCondition() == SearchValue.NOT_BETWEEN)) {
     ...
     * @param encloseColumnName the enclose column name
     * @param sbStringQueryInner the sb string query inner
    ...
    protected StringBuilder appendOrClause(final String oKey, final SearchValue oValue, final boolean encloseColumnName,
        ...
        if ((oSearchValue instanceof List) && !((List<?>) oSearchValue).isEmpty()) {
            ...
            for (int vs = 0; vs < vSearchValues.size(); vs++) {
                ...
                sbStringQueryInner = this.addColumnValue(sbStringQueryInner, oKey, vSearchValues.get(vs),
                        lastOperatorToAppend, encloseColumnName);
     ...
     * @return the string builder
     */
    protected StringBuilder appendInClause(final String oKey, final SearchValue oValue, final boolean encloseColumnName,
            final StringBuilder sbStringQueryInner) {
        return this.appendInAndExistsClause(oKey, oValue.getValue(),
                SearchValue.conditionIntToStr(oValue.getCondition()), encloseColumnName, sbStringQueryInner);
    }
     ...
     * @return the string builder
     */
    protected StringBuilder appendExistsClause(final String oKey, final SearchValue oValue,
            final boolean encloseColumnName, final StringBuilder sbStringQueryInner) {
        return this.appendInAndExistsClause(oKey, oValue.getValue(),
                SearchValue.conditionIntToStr(oValue.getCondition()), encloseColumnName, sbStringQueryInner);
    }
     ...
     * @return the string builder
     */
    protected StringBuilder appendBetweenClause(final String oKey, final SearchValue oValue,
            final boolean encloseColumnName, StringBuilder sbStringQueryInner) {
        final Object oSearchValue = oValue.getValue();
        if ((oSearchValue instanceof List) && (((List<?>) oSearchValue).size() == 2)
                && (((List<?>) oSearchValue).get(0) != null) && (((List<?>) oSearchValue).get(1) != null)) {
            sbStringQueryInner = this.encloseColumnName(sbStringQueryInner, oKey, encloseColumnName);
            sbStringQueryInner.append(' ');
            sbStringQueryInner.append(SearchValue.conditionIntToStr(oValue.getCondition()));
            sbStringQueryInner.append(' ');
    ...
    protected StringBuilder addValueBetweenParenthesis(final StringBuilder sbStringQuery,
            final StringBuilder objectValue) {
        StringBuilder sbStringQueryInner = sbStringQuery;
        sbStringQueryInner.append(SQLStatementBuilder.OPEN_PARENTHESIS);
    ...
    protected StringBuilder addColumnValue(final StringBuilder sbStringQuery, final String oKey,
            final Object objectValue, final String lastOperatorToAppend,
            final boolean encloseColumnName) {
        StringBuilder sbStringQueryInner = sbStringQuery;
        if ((objectValue != null) && !(objectValue instanceof NullValue)) {
            if (objectValue instanceof List) {
                this.appendInAndExistsClause(oKey, objectValue, SearchValue.conditionIntToStr(SearchValue.IN),
                        encloseColumnName, sbStringQueryInner);
            } else {
                sbStringQueryInner = this.encloseColumnName(sbStringQueryInner, oKey, encloseColumnName);
    ...
    /**
     * Check if column name must be enclosed. Returns true if square brackets are necessary insert in
     * this column name.
     * <p>
    ...
    /**
     * Enclose column name.
     * @param sbStringQuery the sb string query
     * @param oKey the o key
     * @param encloseColumnName the enclose column name
     * @return the string builder
     */
    protected StringBuilder encloseColumnName(final StringBuilder sbStringQuery, final String oKey,
            final boolean encloseColumnName) {
