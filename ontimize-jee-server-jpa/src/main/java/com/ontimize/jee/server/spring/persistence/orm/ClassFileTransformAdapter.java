    ...
    public byte[] transform(
        ...
        try {
            ...
            if ((transformed != null) && ClassFileTransformerAdapter.logger.isDebugEnabled()) {
                ClassFileTransformerAdapter.logger.debug("Transformer of class [" + this.classTransformer.getClass().getName() +
                        "] transformed class [" + className + "]; bytes in=" +
                        classfileBuffer.length + "; bytes out=" + transformed.length);
            }
            return transformed;
        } catch (ClassCircularityError ex) {
            ClassFileTransformerAdapter.logger.error("Error weaving class [" + className + "] with " +
            ...
            if (ClassFileTransformerAdapter.logger.isWarnEnabled()) {
                ClassFileTransformerAdapter.logger.warn("Error weaving class [" + className + "] with " +
                        "transformer of class [" + this.classTransformer.getClass().getName() + "]", ex);
            }
