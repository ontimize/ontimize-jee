/*
 * Copyright 2002-2012 the original author or authors. Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in compliance with the
...
/**
 * Simple adapter that implements the {@code java.lang.instrument.ClassFileTransformer} interface based on a JPA ClassTransformer which a JPA PersistenceProvider asks the
...
class ClassFileTransformerAdapter implements ClassFileTransformer {
	...
	public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) {
		...
		try {
			...
			if ((transformed != null) && ClassFileTransformerAdapter.logger.isDebugEnabled()) {
				ClassFileTransformerAdapter.logger.debug("Transformer of class [" + this.classTransformer.getClass()
				        .getName() + "] transformed class [" + className + "]; bytes in=" + classfileBuffer.length + "; bytes out=" + transformed.length);
			}
			return transformed;
		} catch (ClassCircularityError ex) {
			ClassFileTransformerAdapter.logger.error("Error weaving class [" + className + "] with " + "transformer of class [" + this.classTransformer.getClass().getName() + "]",
			        ex);
			throw new IllegalStateException("Could not weave class [" + className + "]", ex);
		} catch (Exception ex) {
			if (ClassFileTransformerAdapter.logger.isWarnEnabled()) {
				ClassFileTransformerAdapter.logger
				        .warn("Error weaving class [" + className + "] with " + "transformer of class [" + this.classTransformer.getClass().getName() + "]", ex);
			}
