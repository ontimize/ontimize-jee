...
public final class MappingInfoUtils {
            ...
            if ((cm.getDbColumn() == null) || cm.getDbColumn().trim().isEmpty()) {
                ...
                if (cm.getBeanAttribute() == null) {
                    if (((cm.getBeanAttributeGetterMethod() == null) || cm.getBeanAttributeGetterMethod().trim().isEmpty())
                            && ((cm.getBeanAttributeSetterMethod() == null) || cm.getBeanAttributeSetterMethod().trim().isEmpty())) {
                        throw new RuntimeException("Bean-attribute and/or bean-atribute-getter-method and/or bean-atribute-setter-method for column mapping for entity '"
                    ...
                    if (getterFound && !setterFound) {
                        throw new RuntimeException("Wrong column mapping (dbcolumn='" + cm.getDbColumn() + "'), no method found for entity '" + entityClazz
                                + "' that matches setter method '" + setter + "' with one argument");
                    } else if (!getterFound && setterFound) {
                        throw new RuntimeException("Wrong column mapping (dbcolumn='" + cm.getDbColumn() + "'), no method found for entity '" + entityClazz
                                + "' that matches getter method '" + getter + "' with none argument, and this field is insertable or updatable");
                    } else if (!getterFound && !setterFound) {
                        throw new RuntimeException("Wrong column mapping (dbcolumn='" + cm.getDbColumn() + "'), any method found for entity '" + entityClazz
                                + "' that matches neither setter method '" + setter + "' with one argument, nor getter method '" + getter + "' with none argument");
                    }
    ...
    public static Boolean isPrimitive(final String returnType2) {
        try {
            ...
            if (Integer.class.isAssignableFrom(forName)
                    || Long.class.isAssignableFrom(forName)
                    || Double.class.isAssignableFrom(forName)
                    || Float.class.isAssignableFrom(forName)
                    || Short.class.isAssignableFrom(forName)
                    || Character.class.isAssignableFrom(forName)
                    || String.class.isAssignableFrom(forName)
                    || Date.class.isAssignableFrom(forName)
                    || Byte.class.isAssignableFrom(forName)
