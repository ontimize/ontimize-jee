...
public final class MappingInfoUtils {
     ...
     * @throws RuntimeException de runtime exception
     */
    public static void validateEntityDefinicion(final QueryType entityDefinition, final String entityClazz)
        ...
        try {
            ...
            throw new RuntimeException(
                    "Entity definition has its clazz wrongly specified '" + entityClazz + "' it does not exists!", e);
        }
        ...
        for (final ColumnMapping cm : columnMapping) {
            MappingInfoUtils.validateColumnDefinicion(entityClazz, entityClass, cm);
    ...
    private static void validateColumnDefinicion(final String entityClazz, Class<?> entityClass,
            final ColumnMapping cm) {
        if ((cm.getDbColumn() == null) || cm.getDbColumn().trim().isEmpty()) {
            throw new RuntimeException(
                    "DBColumn for column mapping for entity '" + entityClazz + "' must be specified");
        }
        if (cm.getBeanAttribute() == null) {
            if (((cm.getBeanAttributeGetterMethod() == null) || cm.getBeanAttributeGetterMethod().trim().isEmpty())
                    && ((cm.getBeanAttributeSetterMethod() == null) || cm
                        .getBeanAttributeSetterMethod()
                        .trim()
                        .isEmpty())) {
                throw new RuntimeException(
                        "Bean-attribute and/or bean-atribute-getter-method and/or bean-atribute-setter-method for column mapping for entity '"
                                + entityClazz + "', must be specified");
            }
        } else if (cm.getBeanAttribute().trim().isEmpty()) {
            throw new RuntimeException("Bean-attribute for column mapping for entity '" + entityClazz
                    + "', if attribute is specified, then it must contain something");
        } else {
            MappingInfoUtils.validateColumnDefinicionGetterSetter(entityClazz, entityClass, cm);

        }
    }

    private static void validateColumnDefinicionGetterSetter(final String entityClazz, Class<?> entityClass,
            final ColumnMapping cm) {
        final String getter = MappingInfoUtils.buildGetterMethodName(cm.getBeanAttribute().trim(),
                cm.getBeanAttributeGetterMethod());
        final String setter = MappingInfoUtils.buildSetterMethodName(cm.getBeanAttribute().trim(),
                cm.getBeanAttributeSetterMethod());
        boolean getterFound = false;
        boolean setterFound = false;
        final Method[] methods = entityClass.getMethods();
        for (final Method m : methods) {
            if (!getterFound && m.getName().equals(getter)
                    && ((m.getParameterTypes() == null) || (m.getParameterTypes().length == 0))) {
                getterFound = true;
            }
            if (!setterFound && m.getName().equals(setter) && (m.getParameterTypes() != null)
        ...
        if (getterFound && !setterFound) {
            throw new RuntimeException("Wrong column mapping (dbcolumn='" + cm
                .getDbColumn() + "'), no method found for entity '" + entityClazz + "' that matches setter method '"
                    + setter + "' with one argument");
        } else if (!getterFound && setterFound) {
            throw new RuntimeException("Wrong column mapping (dbcolumn='" + cm
                .getDbColumn() + "'), no method found for entity '" + entityClazz + "' that matches getter method '"
                    + getter + "' with none argument, and this field is insertable or updatable");
        } else if (!getterFound && !setterFound) {
            throw new RuntimeException("Wrong column mapping (dbcolumn='" + cm
                .getDbColumn() + "'), any method found for entity '" + entityClazz
                    + "' that matches neither setter method '" + setter + "' with one argument, nor getter method '"
                    + getter + "' with none argument");
        }
    }
     ...
     */
    public static MappingInfo buildMappingInfo(final QueryType query, final Class<?> entityClass,
            final Class<?> returnType) {
        MappingInfo mappingInfo = null;
        try {
            mappingInfo = new MappingInfo();
            if (query.getReturn() != null) {
                if ((query.getReturn().getReturnType() != null) && (returnType != null)
                        && returnType.isAssignableFrom(Class.forName(query.getReturn().getReturnType()))) {
                    mappingInfo.setReturnTypeClassName(query.getReturn().getReturnType());
                    mappingInfo.setReturnTypeClassNameIsPrimitive(
                            MappingInfoUtils.isPrimitive(query.getReturn().getReturnType()));
                    ...
                    throw new IllegalArgumentException("return type '" + returnType.getCanonicalName()
                            + "' is not assignable from '" + query.getReturn().getReturnType() + "'");
                }
                ...
                if (returnType.isAssignableFrom(entityClass)) {
                    ...
                    throw new IllegalArgumentException("return type '" + returnType.getCanonicalName()
                            + "' is not assignable from '" + entityClass.getCanonicalName() + "'");
                }
            }
            ...
            throw new IllegalArgumentException(
                    "return type '" + query.getReturn().getReturnType() + "' is a not known class '", e);
        }
    }

    /**
     * Chequea si primitive.
     * @param returnType2 return type2
     * @return the boolean
     */
    public static Boolean isPrimitive(final String returnType2) {
        try {
            ...
            if (Integer.class.isAssignableFrom(forName) || Long.class.isAssignableFrom(forName)
                    || Double.class.isAssignableFrom(forName) || Float.class
                        .isAssignableFrom(forName)
                    || Short.class.isAssignableFrom(forName) || Character.class.isAssignableFrom(forName)
                    || String.class
                        .isAssignableFrom(forName)
                    || Date.class.isAssignableFrom(forName) || Byte.class.isAssignableFrom(forName)
                    || Boolean.class.isAssignableFrom(forName)) {
                return true;
            }
            return false;
        } catch (final ClassNotFoundException e) {
     ...
     * @param entityAttributeSetterMethod entity attribute setter method
     * @return the string
     */
     ...
     * @param entityAttribute entity attribute
     * @param entityAttributeGetterMethod entity attribute getter method
     * @return the string
     */
