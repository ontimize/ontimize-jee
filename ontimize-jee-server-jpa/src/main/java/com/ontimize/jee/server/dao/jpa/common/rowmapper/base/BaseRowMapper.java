...
public class BaseRowMapper<T, S> implements IRowMapper<S> {
	 ...
	 */
	public BaseRowMapper(final MappingInfo mappingInfo, final Class<T> entityClass, final Class<S> returnType,
	        final IColumnTypeToResultSetGetterMethodConverter columnTypeToResultSetGetterMethodConverter, final IDbObjectToJavaObjectConverter dbObjectToJavaObjectConverter) {
		this.entityClass = entityClass;
		this.returnType = returnType;
		this.mappingInfo = mappingInfo;
			...
			if (this.mappingInfo != null) {
				try {
					if ((this.mappingInfo.getReturnTypeClassNameIsPrimitive() != null) && !this.mappingInfo.getReturnTypeClassNameIsPrimitive()) {
						final Object targetEntityObject = this.buildInstanceIfPossible(this.mappingInfo.getReturnTypeClassName());
					...
					final String getterMethod = this.buildResultSetGetterMethodForType(returnTypePrimitiveColumnMappingType);
	...
	public S mapObjectArray(final Object[] data, final List<String> columnNames) throws SQLException {
		if (this.entityClass != null) {
			if (this.mappingInfo != null) {
				...
				if (!this.allowPartialBeanMapping && ((this.mappingInfo.getReturnTypePrimitiveColumnMapping() == null) || this.mappingInfo.getReturnTypePrimitiveColumnMapping()
				        .isEmpty()) && ((data.length != columnMappings.size()) || ((columnNames.size() > 0) && (columnNames.size() != data.length)))) {
				...
				try {
					if ((this.mappingInfo.getReturnTypeClassNameIsPrimitive() != null) && !this.mappingInfo.getReturnTypeClassNameIsPrimitive()) {
						...
						this.buildEntityFromObjectArray(data, columnNames, this.mappingInfo, this.returnType, targetEntityObject);
	 ...
	 */
	private void buildEntityFromRow(final ResultSet rs, final int rowNum, final MappingInfo mappingInfo, final Class<S> targetEntityClass, final Object targetEntityObject)
		...
		if (mappingInfo.getColumnMappings() != null) {
			for (final ColumnMapping cm : mappingInfo.getColumnMappings()) {
				this.apply(rs, rowNum, this.buildResultSetGetterMethodForType(cm.getDbColumnType()), cm.getDbColumn(), targetEntityClass, targetEntityObject,
	 ...
	 */
	private void buildEntityFromObjectArray(final Object[] data, final List<String> columnNames, final MappingInfo mappingInfo, final Class<S> targetEntityClass,
		...
		if (mappingInfo.getColumnMappings() != null) {
			...
			for (final ColumnMapping cm : mappingInfo.getColumnMappings()) {
				...
				if (doIt) {
					this.apply(dataObject, this.buildResultSetGetterMethodForType(cm.getDbColumnType()), cm.getDbColumn(), targetEntityClass, targetEntityObject,
					        MappingInfoUtils.buildSetterMethodName(cm.getBeanAttribute(), cm.getBeanAttributeSetterMethod()), cm.getBeanAttributeType());
				}
				i++;
			}
	 ...
	 *             de row mapper exception
	 */
	private void apply(final ResultSet rs, final int rowNum, final String rsMethod, final String columnName, final Class<?> targetClass, final Object targetObject,
	 ...
	 *             the row mapper exception
	 */
	private void apply(final Object dbObject, final String rsMethod, final String columnName, final Class<?> targetClass, final Object targetObject, final String targetMethod,
	 ...
	 *             de row mapper exception
	 */
	private <R> R getObjectConverted(final ResultSet rs, final int rowNum, final String rsMethod, final String columnName, final Class<R> returnTypeClass)
	        throws NoSuchMethodException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, RowMapperException {
		String getterMethod = null;
		Object dbObject = null;
		try {
			getterMethod = rsMethod;
	 ...
	 *             the row mapper exception
	 */
	private <R> R getObjectConverted(final Object dbObject, final String rsMethod, final String columnName, final Class<R> returnTypeClass)
	...
	protected void handleException(final Exception exception, final String columName, final String rsGetterMethod, final Object columnObject, final String classToReturn) {
		BaseRowMapper.logger.error(ConcatTools.concat("error converting database row to entity:", exception.getMessage(), BaseRowMapper.BREAK_LINE, "Related data: columnName='",
		        columName, "', rsGetterMethod='", rsGetterMethod, "', columnObjectClass='", columnObject != null ? columnObject.getClass().getCanonicalName() : "NULL",
