 ...
 * @param <T>
 *            tipo de la entidad sobre la que se esta realizando la query
 * @param <S>
 *            tipo del resultado de la query
 */
public class BaseRowMapper<T, S> implements IRowMapper<S> {
     ...
     * @param returnType
     *            return type
     * @param columnTypeToResultSetGetterMethodConverter
     *            column type to result set getter method converter
     * @param dbObjectToJavaObjectConverter
     *            db object to java object converter
     */
    public BaseRowMapper(final MappingInfo mappingInfo, final Class<T> entityClass, final Class<S> returnType,
    ...
    public S mapRow(final ResultSet rs, final int rowNum) throws SQLException {
        if (this.entityClass != null) {
            if (this.mappingInfo != null) {
                try {
                    if ((this.mappingInfo.getReturnTypeClassNameIsPrimitive() != null) && !this.mappingInfo.getReturnTypeClassNameIsPrimitive()) {
                        final Object targetEntityObject = buildInstanceIfPossible(this.mappingInfo.getReturnTypeClassName());
                        buildEntityFromRow(rs, rowNum, this.mappingInfo, this.returnType, targetEntityObject);
                        return (S) targetEntityObject;
                    }
                    final String columnName = this.mappingInfo.getReturnTypePrimitiveColumnMapping();
                    final String returnTypePrimitiveColumnMappingType = this.mappingInfo.getReturnTypePrimitiveColumnMappingType();
                    final String getterMethod = buildResultSetGetterMethodForType(returnTypePrimitiveColumnMappingType);
                    return getObjectConverted(rs, rowNum, getterMethod, columnName, this.returnType);
     ...
     * @throws SQLException
     *             the SQL exception
     * @see com.ontimize.jee.server.dao.jpa.common.rowmapper.IRowMapper#mapObjectArray(java.lang.Object[], java.util.List)
     */
    @Override
        ...
        if (this.entityClass != null) {
            if (this.mappingInfo != null) {
                ...
                if (!this.allowPartialBeanMapping
                        && ((this.mappingInfo.getReturnTypePrimitiveColumnMapping() == null) || this.mappingInfo.getReturnTypePrimitiveColumnMapping().isEmpty())
                        && ((data.length != columnMappings.size()) || ((columnNames.size() > 0) && (columnNames.size() != data.length)))) {
                    throw new SQLException(new RowMapperException("the mapping info is incompatible with query result different number of columns"));
                }
                ...
                try {
                    if ((this.mappingInfo.getReturnTypeClassNameIsPrimitive() != null) && !this.mappingInfo.getReturnTypeClassNameIsPrimitive()) {
                        ...
                        buildEntityFromObjectArray(data, columnNames, this.mappingInfo, this.returnType, targetEntityObject);
                    ...
                    final String returnTypePrimitiveColumnMappingType = this.mappingInfo.getReturnTypePrimitiveColumnMappingType();
                    final String getterMethod = buildResultSetGetterMethodForType(returnTypePrimitiveColumnMappingType);
                    return getObjectConverted(data[0], getterMethod, columnName, this.returnType);
                } catch (final RuntimeException e) {
                    throw new SQLException(new RowMapperException(e));
                } catch (final NoSuchMethodException e) {
                    throw new SQLException(new RowMapperException(e));
    ...
    private Object buildInstanceIfPossible(final String returnTypeClassName) throws IllegalArgumentException {
        try {
            ...
            throw new IllegalArgumentException(ConcatTools.concat("specified returnType '", returnTypeName, "' doesn't match sentence return type '", returnTypeClassName, "'"));
        } catch (final InstantiationException e) {
            handleException(e, null, null, null, (String) null);
     ...
     *            target entity class
     * @param targetEntityObject
     *            target entity object
     * @throws NoSuchMethodException
     *             de no such method exception
     * @throws IllegalAccessException
     *             de illegal access exception
     * @throws InvocationTargetException
     *             de invocation target exception
     ...
     */
    private void buildEntityFromRow(final ResultSet rs, final int rowNum, final MappingInfo mappingInfo, final Class<S> targetEntityClass, final Object targetEntityObject) throws NoSuchMethodException, IllegalAccessException,
    InvocationTargetException, ClassNotFoundException, IllegalArgumentException, RowMapperException {
        if (mappingInfo.getColumnMappings() != null) {
            for (final ColumnMapping cm : mappingInfo.getColumnMappings()) {
                apply(rs, rowNum, buildResultSetGetterMethodForType(cm.getDbColumnType()), cm.getDbColumn(), targetEntityClass, targetEntityObject,
                        MappingInfoUtils.buildSetterMethodName(cm.getBeanAttribute(), cm.getBeanAttributeSetterMethod()), cm.getBeanAttributeType());
     ...
     * @throws RowMapperException
     *             the row mapper exception
     */
    private void buildEntityFromObjectArray(final Object[] data, final List<String> columnNames, final MappingInfo mappingInfo, final Class<S> targetEntityClass, final Object targetEntityObject) throws NoSuchMethodException, IllegalAccessException,
    InvocationTargetException, ClassNotFoundException, IllegalArgumentException, RowMapperException {
        if (mappingInfo.getColumnMappings() != null) {
            int i = 0;
            for (final ColumnMapping cm : mappingInfo.getColumnMappings()) {
                ...
                if (doIt) {
                    apply(dataObject, buildResultSetGetterMethodForType(cm.getDbColumnType()), cm.getDbColumn(), targetEntityClass, targetEntityObject,
                            MappingInfoUtils.buildSetterMethodName(cm.getBeanAttribute(), cm.getBeanAttributeSetterMethod()), cm.getBeanAttributeType());
                }
                i++;
            }
    ...
    private String buildResultSetGetterMethodForType(final String dbColumnType) {
        if (this.columnTypeToResultSetGetterMethodConverter != null) {
            return this.columnTypeToResultSetGetterMethodConverter.getResultSetGetterMethodFromDBColumnType(dbColumnType);
        }
     ...
     *             de row mapper exception
     */
    private void apply(final ResultSet rs, final int rowNum, final String rsMethod, final String columnName, final Class<?> targetClass, final Object targetObject, final String targetMethod, final String typeClassName)
            throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, ClassNotFoundException, IllegalArgumentException, RowMapperException {
        ...
        try {
            ...
            handleException(e, columnName, getterMethod, dbObject, typeClassName);
            throw e;
     ...
     *             the row mapper exception
     */
    private void apply(final Object dbObject, final String rsMethod, final String columnName, final Class<?> targetClass, final Object targetObject, final String targetMethod, final String typeClassName)
            throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, ClassNotFoundException, IllegalArgumentException, RowMapperException {
        try {
            final Class<?> targetAttributeType = Class.forName(typeClassName);
            final Method method2 = targetClass.getMethod(targetMethod, targetAttributeType);
            ...
            handleException(e, columnName, "", dbObject, typeClassName);
            throw e;
        } catch (final SecurityException e) {
            handleException(e, columnName, "", dbObject, typeClassName);
            throw e;
     ...
     *             de row mapper exception
     */
    private <R> R getObjectConverted(final ResultSet rs, final int rowNum, final String rsMethod, final String columnName, final Class<R> returnTypeClass) throws NoSuchMethodException, IllegalAccessException,
    IllegalArgumentException, InvocationTargetException, RowMapperException {
        String getterMethod = null;
        Object dbObject = null;
        try {
            getterMethod = rsMethod;
            ...
            handleException(e, columnName, getterMethod, dbObject, toCanonicalName(returnTypeClass));
            throw e;
        } catch (final SecurityException e) {
            handleException(e, columnName, getterMethod, dbObject, toCanonicalName(returnTypeClass));
            throw e;
     ...
     *             the row mapper exception
     */
    private <R> R getObjectConverted(final Object dbObject, final String rsMethod, final String columnName, final Class<R> returnTypeClass) throws
    IllegalArgumentException, RowMapperException {
        try {
            ...
            handleException(e, columnName, "", dbObject, toCanonicalName(returnTypeClass));
            throw e;
        } catch (final IllegalArgumentException e) {
            handleException(e, columnName, "", dbObject, toCanonicalName(returnTypeClass));
            throw e;
     ...
     * 
     * @param columnTypeToResultSetGetterMethodConverter
     *            nuevo column type to result set getter method converter
     */
    public void setColumnTypeToResultSetGetterMethodConverter(final IColumnTypeToResultSetGetterMethodConverter columnTypeToResultSetGetterMethodConverter) {
        this.columnTypeToResultSetGetterMethodConverter = columnTypeToResultSetGetterMethodConverter;
    }
     ...
     * 
     * @param dbObjectToJavaObjectConverter
     *            nuevo db object to java object converter
     */
     ...
     *            class to return
     */
    protected void handleException(final Exception exception, final String columName, final String rsGetterMethod, final Object columnObject, final String classToReturn) {
        BaseRowMapper.LOGGER.error(ConcatTools.concat("error converting database row to entity:", exception.getMessage(), BaseRowMapper.BREAK_LINE, "Related data: columnName='", columName, "', rsGetterMethod='", rsGetterMethod
                , "', columnObjectClass='", (columnObject != null ? columnObject.getClass().getCanonicalName() : "NULL"), "', classToReturn='", classToReturn, "'"));
    }

    /**
     * Sets the allow partial bean mapping.
     * 
     * @param allowPartialBeanMapping
     *            the new allow partial bean mapping
     */
