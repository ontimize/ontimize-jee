...
public class BeanPropertyRowMapper<T> implements RowMapper<T> {
        ...
        for (int index = 1; index <= columnCount; index++) {
            ...
            if (pd != null) {
                ...
                if (BeanPropertyRowMapper.logger.isDebugEnabled() && (rowNumber == 0)) {
                    BeanPropertyRowMapper.logger.debug("Mapping column '" + column + "' to property '" + pd.getName()
                ...
                try {
                    ...
                    BeanPropertyRowMapper.logger
                        .error("Intercepted TypeMismatchException for row " + rowNumber + " and column '" + column
                                + "' with value " + value + " when setting property '" + pd
                                    .getName()
     ...
     * {@link JdbcUtils#getResultSetValue(java.sql.ResultSet, int, Class)}. Subclasses may override this
     * to check specific value types upfront, or to post-process values return from
     * {@code getResultSetValue}.
     * @param rs is the ResultSet holding the data
     * @param index is the column index
     * @param pd the bean property that each result object is expected to match (or {@code null} if none
     *        specified)
