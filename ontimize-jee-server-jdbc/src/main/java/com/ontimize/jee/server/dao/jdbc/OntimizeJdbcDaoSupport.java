...
public class OntimizeJdbcDaoSupport extends JdbcDaoSupport implements ApplicationContextAware, IOntimizeDaoSupport {
    ...
    protected int getQueryRecordNumber(Map<?, ?> keysValues, final String queryId) {
        ...
        SQLStatementBuilder.SQLStatement stSQL = null;
        ...
        if (queryTemplateInformation != null) {
            ...
            String sqlTemplate = queryTemplateInformation.getSqlTemplate().replaceAll(OntimizeJdbcDaoSupport.PLACEHOLDER_COLUMNS, SQLStatementBuilder.COUNT);
            // Where
            final List<Object> vValues = new ArrayList<>();
            String cond = this.getStatementHandler().createQueryConditionsWithoutWhere(kvValidKeysValues, new ArrayList<>(), vValues);
            if (cond == null) {
                cond = "";
            }
            cond = cond.trim();
            ...
            stSQL = this.getStatementHandler().createCountQuery(this.getSchemaTable(), new HashMap<>(kvValidKeysValues), new ArrayList<>(), new ArrayList<>());
        }

        String sqlQuery = stSQL.getSQLStatement();
        List vValues = stSQL.getValues();
        EntityResult erResult = this.getJdbcTemplate().query(sqlQuery, new EntityResultResultSetExtractor(this.getStatementHandler(), queryTemplateInformation), vValues.toArray());
     ...
     */
    public SQLStatementBuilder.SQLStatement composeQuerySql(final String queryId, final List<?> attributes, final Map<?, ?> keysValues, final List<?> sort, PageableInfo pageableInfo, ISQLQueryAdapter queryAdapter) {
        final QueryTemplateInformation queryTemplateInformation = this.getQueryTemplateInformation(queryId);
     ...
     * @return the Map
     */
    protected Map<Object, Object> applyTransformations(final QueryTemplateInformation templateInformation, final Map<Object, Object> kvValidKeysValues) {
        final List<AmbiguousColumnType> ambiguousColumns = templateInformation.getAmbiguousColumns();
        final List<FunctionColumnType> functionColumns = templateInformation.getFunctionColumns();

        final Map<Object, Object> res = new HashMap<>();
        for (final Map.Entry<Object, Object> kvEntry : kvValidKeysValues.entrySet()) {
     ...
     * @return the object
     */
    protected SQLStatementBuilder.BasicExpression applyTransformationsToBasicExpression(final SQLStatementBuilder.BasicExpression original, List<AmbiguousColumnType> ambiguousColumns, List<FunctionColumnType> functionColumns) {
     ...
     */
    protected SQLStatementBuilder.BasicField applyTransformationsToBasicField(SQLStatementBuilder.BasicField originalField, List<AmbiguousColumnType> ambiguousColumns, List<FunctionColumnType> functionColumns) {
    ...
    protected Map<String, Object> removeNullValues(Map<String, Object> inputAttributesValues) {
        ...
        for (final Map.Entry<String, ?> entry : inputAttributesValues.entrySet()) {
     ...
     * @return
     */
    protected Map<Object, Object> getValidQueryingKeysValues(Map<Object, Object> inputKeysValues, List<String> validColumns) {
        if ((validColumns == null) || validColumns.isEmpty()) {
            return inputKeysValues;
        }
        final Map<Object, Object> hValidKeysValues = new HashMap<>();
        for (Map.Entry<Object, Object> entry : inputKeysValues.entrySet()) {
     ...
     * @param functionColumns  the function columns
     */
    public void addQueryTemplateInformation(final String id, final String value, final List<AmbiguousColumnType> ambiguousColumns, final List<FunctionColumnType> functionColumns, List<OrderColumnType> orderColumns) {
     ...
     * @param validColumns
     */
    public void addQueryTemplateInformation(final String id, final String value, final List<AmbiguousColumnType> ambiguousColumns, final List<FunctionColumnType> functionColumns, List<String> validColumns, List<OrderColumnType> orderColumns) {
