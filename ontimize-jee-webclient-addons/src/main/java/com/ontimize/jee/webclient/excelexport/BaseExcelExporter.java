...
import com.ontimize.jee.webclient.excelexport.helpers.ExcelHelper;
...
public abstract class BaseExcelExporter<T extends Workbook> implements Exporter<T> {
     ...
     * Podrían venir de las anotaciones en la tabla. Pero podrían estar todas a null. Si no se
     * usaron anotaciones, la forma de asignar estilo es es mediante el StyleProvider. Estos estilos
     * complementan a los que tuvieran las columnas. En el StyleProvider hay tres estilos: -
     * Cabecera: estilo por defecto para las cabeceras. - Columna: estilo de todas las celdas de una
     * columna. - Celda: estilo para celdas concretas. DefaultStyleProvider proporciona por defecto
     * unos estilos para todas las cabeceras y además unos por tipo para todas las columnas. Se
    ...
    protected void addBody(
        ...
        final SheetContext firstSheetContext = new SheetContext(
            ...
            sheetNameProvider.getDefaultSheetName(),
            0);
        // Obtenemos la primera hoja con ese contexto, que será una hoja nueva con el nombre que dió el usuario
        final Sheet firstSheet = this.selectSheetByName(
            sheetNameProvider.getSheetName().call(firstSheetContext),
            dataProvider,
            headerColumns,
            headerCellStylesById,
            exportOptions,
        ...
        IntStream.range(0, dataProvider.getNumberOfRows()).forEach(rowIndex -> {
            ...
            final String newSheetName = sheetNameProvider.getSheetName().call(sheetContext);
