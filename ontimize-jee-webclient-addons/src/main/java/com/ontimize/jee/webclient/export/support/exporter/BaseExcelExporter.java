...
public abstract class BaseExcelExporter<T extends Workbook> implements ExcelExporter<T> {
     ...
     * de columnas. Podrían venir de las anotaciones en la tabla. Pero podrían estar
     * todas a null. Si no se usaron anotaciones, la forma de asignar estilo es es
     * mediante el StyleProvider. Estos estilos complementan a los que tuvieran las
     * columnas. En el StyleProvider hay tres estilos: - Cabecera: estilo por
     * defecto para las cabeceras. - Columna: estilo de todas las celdas de una
     * columna. - Celda: estilo para celdas concretas. DefaultStyleProvider
     * proporciona por defecto unos estilos para todas las cabeceras y además unos
     * por tipo para todas las columnas. Se pueden sobreescribir
     */
    @Override
    public T export(final ExportColumnProvider exportColumnProvider, final ExportDataProvider dataProvider,
            final ExportStyleProvider styleProvider, SheetNameProvider sheetNameProvider, ExportOptions exportOptions)
        ...
        // Primero creamos una hoja "plantilla" con las cabeceras para que todas las
        // hojas
        // adicionales
        ...
        workBook.sheetIterator().forEachRemaining(currentSheet -> bodyColumns.stream()
                .forEach(column -> currentSheet.autoSizeColumn(bodyColumns.indexOf(column))));
    ...
    protected void addBody(final List<ExportColumn> userColumns, final ExportDataProvider dataProvider,
        ...
        // Tomamos el contexto de la primera hoja, que es la del nombre que dió el
        // usuario y de
        // índice 0
        final SheetContext firstSheetContext = new SheetContext(0, 0, sheetNameProvider.getDefaultSheetName(),
                sheetNameProvider.getDefaultSheetName(), 0);
        // Obtenemos la primera hoja con ese contexto, que será una hoja nueva con el
        // nombre que dió el
        // usuario
        final Sheet firstSheet = this.selectSheetByName(sheetNameProvider.getSheetName().apply(firstSheetContext),
                dataProvider, headerColumns, headerCellStylesById, exportOptions, styleProvider);
        ...
        // Para cada fila, obtenemos el contexto de hoja, y si hay que cambiar de hoja
        // cambiamos
        IntStream.range(0, dataProvider.getNumberOfRows()).forEach(rowIndex -> {
            final SheetContext sheetContext = new SheetContext(rowIndex, aLastRowNum.get(),
                    sheetNameProvider.getDefaultSheetName(), aSheetName.get(), aActualSheetIndex.get());
            final String newSheetName = sheetNameProvider.getSheetName().apply(sheetContext);
            final Sheet actualSheet = this.selectSheetByName(newSheetName, dataProvider, headerColumns,
                    headerCellStylesById, exportOptions, styleProvider);
            ...
            userColumns.stream().forEach(column -> {
                ...
                this.addCell(actualSheet, column, aLastRowNum.get(), userColumns.indexOf(column), value, styleProvider,
                        bodyCellStyles);
            });
        });
    }
    ...
    protected void createCell(final Sheet sheet, final ExportColumn exportColumn, final int row, final int colUserIndex,
            final Object cellValue, final DefaultExcelExportStyleProvider styleProvider,
            final Map<String, CellStyle> bodyCellStyles) {
        ...
        this.setCellStyle(sheet, exportColumn, row, colUserIndex, cellValue, (ExportStyleProvider) styleProvider, cell,
                bodyCellStyles);
        this.assignCellValue(cell, cellValue, styleProvider);
    }

    protected void setCellStyle(final Sheet sheet, final ExportColumn<ExportColumnStyle> exportColumn, final int row,
            final int colUserIndex, final Object cellValue,
            final ExportStyleProvider<CellStyle, DataFormat> styleProvider, final Cell cell,
            final Map<String, CellStyle> bodyCellStyles) {
        ...
        final CellStyle userStyle = styleProvider.getCellStyle(
                new CellStyleContext<>(row, colUserIndex, exportColumn.getId(), cellValue, columnStyle, () -> {
    ...
    protected void createHeader(final ExportDataProvider provider, final Sheet sheet,
        ...
        userHeaderColumns.forEach(exportColumn -> {
            final int width = this.createHeader(sheet, exportColumn, 0, columnIndex.get(), headerCellStylesById,
                    styleProvider);
        ...
        userHeaderColumns.stream().filter(t -> t.getHeadExportColumnCount() == 0).forEach(column -> {
...
//                    final int columnNumber = provider.getColumnIndex(exportColumn);
                ...
                if (headerHeight > 0) {
                    final CellRangeAddress region = new CellRangeAddress(0, headerHeight, columnNumber, columnNumber);
...
//                    cell);
    ...
    protected void updateCellStyle(final Sheet sheet, final ExportColumnStyle fieldStyle, final CellStyle cellStyle) {
...
//                    final ExportColumn column = bodyColumns.get(dataProviderColumnIndex);
             ...
             */
            this.exportColumnStyle.set(styleProvider.getColumnStyleByType(objectClass))
                    .set(styleProvider.getColumnStyle(column.getId()));
            final CellStyle cellStyle = this.createCellStyle(sheet, this.exportColumnStyle);
...
//                }
    ...
    private void applyStyleToHeaderCell(final Sheet sheet, final HeadExportColumn column, final int rowIndex,
            final int columnIndex, final Map<String, CellStyle> headerCellStylesById,
            final ExportStyleProvider styleProvider, final Cell cell) {
        ...
        final CellStyle userStyle = (CellStyle) styleProvider.getHeaderCellStyle(new CellStyleContext<>(rowIndex,
                columnIndex, column.getId(), cell.getStringCellValue(), style, cellStyleSupplier, dataFormatSupplier));
        if (userStyle != null) {
            cell.setCellStyle(userStyle);
        }
    }
    ...
    private void assignCellValue(final Cell cell, final Object value,
        ...
        ExportColumnStyle booleanCellRenderStyle = styleProvider.getStyles().get("booleanCellRender");

        String trueValue = (booleanCellRenderStyle != null && booleanCellRenderStyle.getTrueValue() != null)
                ? booleanCellRenderStyle.getTrueValue()
                : "Verdadero";
        String falseValue = (booleanCellRenderStyle != null && booleanCellRenderStyle.getFalseValue() != null)
                ? booleanCellRenderStyle.getFalseValue()
                : "Falso";
        if (Number.class.isAssignableFrom(value.getClass())) {
            ...
            if (booleanCellRenderStyle != null) {
                cell.setCellValue((Boolean) value ? trueValue : falseValue);
            } else {
    ...
    private void addHeaderStyles(final Sheet sheet, final HeadExportColumn column,
         ...
         */
        if (finalStyle != null) {
            headerCellStylesById.put(column.getId(), this.createCellStyle(sheet, finalStyle));
        }
        /*
        ...
        if (column.getHeadExportColumnCount() > 0) {
            for (int n = 0; n < column.getHeadExportColumnCount(); n++) {
                this.addHeaderStyles(sheet, column.getHeadExportColumn(n), headerCellStylesById, styleProvider);
            }
        }
    }
