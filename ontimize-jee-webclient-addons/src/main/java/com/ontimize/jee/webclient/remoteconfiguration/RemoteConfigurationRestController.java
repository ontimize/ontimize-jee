...
public abstract class RemoteConfigurationRestController<T extends IRemoteConfigurationService, S extends IRemoteConfigurationNameConverter> {
	...
	protected void checkRemoteConfigurationParams(Map<?, ?> values) {
		String userColumn = this.remoteConfigurationNameConverter != null ? this.remoteConfigurationNameConverter.getUserColumn() : IRemoteConfigurationDao.DEFAULT_COLUMN_USER;
		CheckingTools.failIf(!values.containsKey(userColumn), OntimizeJEERuntimeException.class, "ERROR_PARAM_" + userColumn + "_NOT_FOUND");
		String appColumn = this.remoteConfigurationNameConverter != null ? this.remoteConfigurationNameConverter.getAppColumn() : IRemoteConfigurationDao.DEFAULT_COLUMN_APP;
		CheckingTools.failIf(!values.containsKey(appColumn), OntimizeJEERuntimeException.class, "ERROR_PARAM_" + appColumn + "_NOT_FOUND");
	}
