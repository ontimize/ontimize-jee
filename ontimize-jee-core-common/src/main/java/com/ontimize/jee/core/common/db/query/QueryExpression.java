...
import com.ontimize.jee.core.common.db.SQLStatementBuilder.Expression;
...
public class QueryExpression implements Serializable {
    ...
    public QueryExpression(Expression ex, String entity, String[] cols, boolean[] queryCols) {
        this.expression = ex;
        if (cols != null) {
            this.cols = cols;
        } else {
    ...
    public QueryExpression(Expression ex, String entity, List cols, List queryCols) {
    ...
    public boolean equals(Object e) {
        ...
        if (e instanceof Expression) {
            if (((Expression) e).getLeftOperand() == null) {
                return false;
            }
            if (((Expression) e).getRightOperand() == null) {
                return false;
            }
            if (((Expression) e).getOperator() == null) {
                return false;
            }

            return this.expression.getLeftOperand().toString().equals(((Expression) e).getLeftOperand().toString())
                    && this.expression.getRightOperand()
                        .toString()
                        .equals(((Expression) e).getRightOperand().toString())
                    && this.expression.getOperator().toString().equals(((Expression) e).getOperator().toString());
        } else {
            return false;
        }
    }
