...
public class ExtendedRemoteReferencesXmlParser extends ExtendedXmlParser {
    ...
    protected void executeOperation(Document originalForm, ExtendedXmlOperation operation) throws Exception {
        ...
        if (operation.getOperation().equals(ExtendedXmlParser.ADD_REMOTE_REFERENCE)) {
        ...
        if (foundNode != null) {
            ...
            if (operationType.equalsIgnoreCase(ExtendedXmlParser.ADD_REMOTE_REFERENCE)) {
                appendAddRemoteReference(originalForm, operation, foundNode);
            }

            if (operationType.equalsIgnoreCase(ExtendedXmlParser.REMOVE_REMOTE_REFERENCE)) {
                Node parentNode = foundNode.getParentNode();
                parentNode.removeChild(foundNode);
            }

            if (operationType.equalsIgnoreCase(ExtendedXmlParser.MODIFY_REMOTE_REFERENCE)) {
    ...
    protected void modifyGroupOperationsNode(Document originalForm, Element foundNode,
        ...
        if (currentGroupOperationsNode.getNodeName().equalsIgnoreCase(ExtendedXmlParser.MODIFY_REFERENCE_MODIFY_INIT)) {
            foundNode.setAttribute(ExtendedXmlParser.MODIFY_REFERENCE_INIT,
                    currentGroupOperationsNode.getAttributes()
                        .getNamedItem(ExtendedXmlParser.MODIFY_ATTRIBUTE_VALUE)
                        .getNodeValue());
        }
        if ((currentGroupOperationsNode.getNodeType() == Node.ELEMENT_NODE) && !currentGroupOperationsNode.getNodeName()
            .equalsIgnoreCase(ExtendedXmlParser.MODIFY_REFERENCE_PARAMS)) {
            ...
            for (int j = 0; j < foundElementList.getLength(); j++) {
                ...
                if (currentGroupOperationsNode.getNodeName()
                    .equalsIgnoreCase(ExtendedXmlParser.MODIFY_REFERENCE_MODIFY_NAME)
                        && currentElement.getNodeName()
                            .equals(ExtendedXmlParser.MODIFY_REFERENCE_NAME)) {
                    ...
                    .equalsIgnoreCase(ExtendedXmlParser.MODIFY_REFERENCE_MODIFY_CLASS)
                        && currentElement.getNodeName()
                            .equals(ExtendedXmlParser.MODIFY_REFERENCE_CLASS)) {
                    Node childElementText = currentElement.getFirstChild();
                    childElementText.setNodeValue(currentGroupOperationsNode.getAttributes()
                        .getNamedItem(ExtendedXmlParser.MODIFY_ATTRIBUTE_VALUE)
                        .getNodeValue());
    ...
    protected void modifyReferenceParameters(Document originalForm, Element foundNode,
        ...
        for (int j = 0; j < paramNodesOperation.getLength(); j++) {
            ...
            if (paramNodeOperation.getNodeType() == Node.ELEMENT_NODE) {
                ...
                String nodeName = ((Element) paramNodeOperation).getAttribute(ExtendedXmlParser.MODIFY_ATTRIBUTE_NAME)
                    .toString();
                String nodeValue = ((Element) paramNodeOperation).getAttribute(ExtendedXmlParser.MODIFY_ATTRIBUTE_VALUE)
                    .toString();
                String nodeOperation = ((Element) paramNodeOperation)
                    .getAttribute(ExtendedXmlParser.MODIFY_ATTRIBUTE_OPERATION);
                String nodeSeparator = ((Element) paramNodeOperation)
                    .getAttribute(ExtendedXmlParser.MODIFY_ATTRIBUTE_SEPARATOR);

                Node paramsToModify = this.searchNodeName(nodeName, foundNode);

                if ((nodeOperation.equalsIgnoreCase(ExtendedXmlParser.MODIFY_ATTRIBUTE_OPERATOR_ADD)
                        && (paramsToModify == null))
                        || (nodeOperation
                ...
                } else if (nodeOperation.equalsIgnoreCase(ExtendedXmlParser.MODIFY_ATTRIBUTE_OPERATOR_ADD)
                        && (paramsToModify != null)) {
                    modifyAttributeOperatorAdd(nodeValue, nodeSeparator, paramsToModify);

                } else if (nodeOperation.equalsIgnoreCase(ExtendedXmlParser.MODIFY_ATTRIBUTE_OPERATOR_REMOVE)) {
                    ...
                    NodeList valueToModify = ((Element) paramsToModify)
                        .getElementsByTagName(ExtendedXmlParser.MODIFY_REFERENCE_VALUE);
    ...
    protected Node searchNodeName(String name, Element foundNode) {
        NodeList paramsNodesDocument = foundNode.getElementsByTagName(ExtendedXmlParser.MODIFY_REFERENCE_NAME);
