...
public class OntimizeCassandraDaoSupport implements IOntimizeDaoSupport, ApplicationContextAware {
	...
	/**
	 * Has this operation been compiled? Compilation means at least checking that a CqlTemplate has been provided, but subclasses may also implement their own custom validation.
	 */
	...
	protected CQLStatement composeCQLQuery(final String queryId, final List<?> attributes, final Map<?, ?> keysValues, final List<?> sort, ISQLQueryAdapter adapter) {
		...
		if (cassandraQueryInformation == null) {
			...
			// use table
			cqlStatement = this.getStatementHandler().createSelectQuery(this.getKeyspaceTableName(), vValidAttributes, kvValidKeysValues, new ArrayList<String>(),
					(List<SQLOrder>) sort);
		}
	...
	public Map<String, Object> getValidAttributes(final Map<?, ?> inputAttributesValues) {
		...
		OntimizeCassandraDaoSupport.logger.debug(" Update valid attributes values: Input: " + inputAttributesValues + " -> Result: " + hValidKeysValues);
		return hValidKeysValues;
	}
	...
	public synchronized final void compile() throws InvalidDataAccessApiUsageException {
		if (!this.isCompiled()) {
			...
			if (OntimizeCassandraDaoSupport.logger.isDebugEnabled()) {
				OntimizeCassandraDaoSupport.logger.debug("{} for table [ {} ] compiled", this.getClass(), this.getTableName());
			}
		}
	}
	 ...
	 * @param validColumns
	 */
	public void addCassandraQueryTemplateInformation(final String id, final String value, final List<AmbiguousColumnType> ambiguousColumns,
			final List<FunctionColumnType> functionColumns, List<String> validColumns) {
		this.cqlQueries.put(id, new CassandraQueryTemplateInformation(value, ambiguousColumns, functionColumns, validColumns));
	}
	...
	public Map<String, Object> getValidUpdatingKeysValues(final Map<?, ?> keysValues) {
		...
		OntimizeCassandraDaoSupport.logger.debug(" Update valid keys values: Input: " + keysValues + " -> Result: " + hValidKeysValues);
		return hValidKeysValues;
	}
