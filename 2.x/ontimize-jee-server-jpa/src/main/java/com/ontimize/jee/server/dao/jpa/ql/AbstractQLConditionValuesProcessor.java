...
public abstract class AbstractQLConditionValuesProcessor implements QLConditionValuesProcessor {
	...
	/**
	 * Creates a <code>AbstractQLConditionValuesProcessor</code> where every condition that uses <code>LIKE</code> is case-insensitive. <p> Inserts a upper function in both sides
	...
	/**
	 * Creates a <code>AbstractQLConditionValuesProcessor</code> where every condition that uses <code>LIKE</code> or is a column of String type is case-insensitive. <p> Inserts a
	...
	/**
	 * Check if column name must be enclosed. Returns true if square brackets are necessary insert in this column name. <p> If the column name has a character of the special
	 * character list, it's necessary insert column name in square brackets.
	 *
	...
	/**
	 * Adds characters to the list of special characters. <p> When the query is be created if a column name contains a character of the list this column name is inserted between
