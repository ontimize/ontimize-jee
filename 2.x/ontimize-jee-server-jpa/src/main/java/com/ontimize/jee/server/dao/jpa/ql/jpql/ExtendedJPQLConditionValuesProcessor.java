...
public class ExtendedJPQLConditionValuesProcessor extends DefaultJPQLConditionValuesProcessor {
	...
	public ExtendedJPQLConditionValuesProcessor() {
		super(false);
	}

	/**
	 * Creates a <code>ExtendedSQLConditionValuesProcessor</code> where every condition that uses <code>LIKE</code> is case-insensitive. <p> Inserts a upper function in both sides
	 * of <code>LIKE</code> conditions (UPPER(Field) LIKE UPPER(Value))
	 *
	 * @param upperLike
	 *            true if the LIKE condition should be case-insensitive
	 */

	public ExtendedJPQLConditionValuesProcessor(final boolean upperLike) {
		super(upperLike);
	}

	/**
	 * Creates a <code>ExtendedSQLConditionValuesProcessor</code> where every condition that uses <code>LIKE</code> or is a column of String type is case-insensitive. <p> Inserts a
	 * upper function in both sides of <code>LIKE</code> conditions (UPPER(Field) LIKE UPPER(Value)) Inserts a upper function in both sides if column type is a String (UPPER(Field)
	 * LIKE UPPER(String))
	 *
	 * @param upperStrings
	...
	public ExtendedJPQLConditionValuesProcessor(final boolean upperStrings, final boolean upperLike) {
	 ...
	 *
	 * @param name
	 *            column name to checks
	 * @return the column name in square bracket if it's necessary
	 */
	protected String getColumnName(final String name) {
		String columnName = name;
		if (this.checkColumnName(name)) {
			columnName = SQLStatementBuilder.OPEN_SQUARE_BRACKET + columnName + SQLStatementBuilder.CLOSE_SQUARE_BRACKET;
		}
		if (this.beanPrefix != null) {
			columnName = this.beanPrefix + '.' + columnName;
		}
		return columnName;
	}
	 ...
	 *
	 * @param expression
	 *            a class that implements <code>Expression<code> interface
	 * @param values
	...
	/**
	 * Creates the condition string from a expression. The condition string is stored in the <code>StringBuilder</code> param and the required values in a <code>Vector</code> param
	 *
	 * @param expression
	 *            a class that implements <code>Expression<code> interface
	 * @param values
	...
	private StringBuilder createQueryConditionsFromExpression(final Expression expression, final StringBuilder sb) {
		...
		if (lo instanceof Field) {
			...
			if ((BasicOperator.LIKE_OP.equals(expressionOperator) || BasicOperator.NOT_LIKE_OP.equals(expressionOperator)) && this.isUpperLike()) {
			...
			if (ro != null) {
				if (ro instanceof Expression) {
					...
					if (BasicOperator.LIKE_OP.equals(expressionOperator) || BasicOperator.NOT_LIKE_OP.equals(expressionOperator)) {
						if (this.isUpperLike()) {
	 ...
	 *            a condition list
	 * @param wildcards
	 *            column list that can use wildcards
	 * @param values
	 *            vector where the value of each processed conditions is stored
	 * @return
	 */
	...
	public String createQueryConditions(final Map<?, ?> conditions, final List<String> wildcards) {
		...
		if (conditions
		        .containsKey(ExtendedSQLConditionValuesProcessor.EXPRESSION_KEY) && (conditions.get(ExtendedSQLConditionValuesProcessor.EXPRESSION_KEY) instanceof Expression)) {
