...
public class XMLLevelManagerBuilder extends XMLInterpreter implements com.ontimize.jee.desktopclient.components.treetabbedformmanager.levelmanager.builder.LevelManagerBuilder {
	...
	private void preprocessChildren(List<CustomNode> unprocessedChildNodes, List<CustomNodeWrapper> preProcessedNodes, CustomNodeWrapper parentPreprocessedNode,
		...
		for (int i = 0; i < unprocessedChildNodes.size(); i++) {
			if (unprocessedChildNodes.get(i).isTag()) {
				...
				} else if ((parentPreprocessedNode != null) && (parentPreprocessedNode.getAttributes().get(Level.NEXT_LEVEL) != null) && !parentPreprocessedNode.getAttributes()
				        .get(Level.NEXT_LEVEL).equals(attributes.get(Level.ID))) {
