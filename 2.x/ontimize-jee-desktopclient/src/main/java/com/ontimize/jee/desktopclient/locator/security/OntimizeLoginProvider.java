...
public class OntimizeLoginProvider implements ILoginProvider {
	...
	private String getUserServiceUrl() {
		...
		// no pedemos autenticar con un servicio hessian porque la entity (inputstream) no es "repeatable" y no puede llevar a cabo la negociacion
		String serviceUrl = ((OntimizeHessianProxyFactoryBean) ((Advised) userService).getAdvisors()[0].getAdvice()).getServiceUrl();
		return serviceUrl;
	}
