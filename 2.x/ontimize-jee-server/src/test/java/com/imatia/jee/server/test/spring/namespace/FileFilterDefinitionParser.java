...
/**
 * Creates a list of FileFilters based on the configured value, ref, idRef, or bean. <p/>
 *
 * Note: value defaults to a NamedFileFilter
 *
 * Example:
 *
 * Create a NamedFileFilter that will filter for the specific name <core-commons:fileFilter id="a"> <value>someFile.txt</value> </core-commons:fileFilter>
 *
 * Create a filter that will filter for any file ending in .xml and that is writable <core-commons:fileFilter id="a"> <bean
	...
	protected void doParse(Element element, ParserContext ctx, BeanDefinitionBuilder builder) {
		...
		try {
			...
			for (int i = 0; i < nl.getLength(); i++) {
				Object ob = DefinitionParserUtil.parseNode(nl.item(i), ctx, builder.getBeanDefinition(), element.getAttribute("scope"));
				if (ob != null) {
					filters.add(ob);
				}
			}
