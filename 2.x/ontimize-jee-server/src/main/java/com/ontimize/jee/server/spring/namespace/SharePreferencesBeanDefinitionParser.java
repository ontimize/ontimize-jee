...
public class SharePreferencesBeanDefinitionParser extends AbstractSingleBeanDefinitionParser {
	...
	protected void doParse(Element element, ParserContext ctx, BeanDefinitionBuilder builder) {
		...
		Element item = DomUtils.getChildElementByTagName(element, SharePreferencesBeanDefinitionParser.SHARE_PREF_ENGINE_PROPERTY);
		Element child = DomUtils.getChildElements(item).get(0);
		Object engine = null;

		if (SharePreferencesBeanDefinitionParser.SHARE_PREF_DATABASE_CONFIGURATION_PROPERTY.equals(child.getLocalName())) {
			...
			engine = DefinitionParserUtil.parseNode(child, ctx, builder.getBeanDefinition(), element.getAttribute(SharePreferencesBeanDefinitionParser.SCOPE), false);
		}
	...
	public static class DatabaseSharePreferencesParser extends AbstractSingleBeanDefinitionParser {
		...
		protected void doParse(final Element element, final ParserContext ctx, final BeanDefinitionBuilder builder) {
			builder.addPropertyReference("daoSharePref", DefinitionParserUtil.nullIfEmpty(element.getAttribute(SharePreferencesBeanDefinitionParser.SHARE_PREF_REPOSITORY)));
			builder.addPropertyReference("daoSharePrefTarget",
					DefinitionParserUtil.nullIfEmpty(element.getAttribute(SharePreferencesBeanDefinitionParser.SHARE_PREF_TARGET_REPOSITORY)));

			builder.addPropertyValue("shareKeyColumn", DefinitionParserUtil.nullIfEmpty(element.getAttribute(SharePreferencesBeanDefinitionParser.SHARE_PREF_KEY_COLUMN)));
			builder.addPropertyValue("shareTargetKeyColumn",
					DefinitionParserUtil.nullIfEmpty(element.getAttribute(SharePreferencesBeanDefinitionParser.SHARE_PREF_TARGET_KEY_COLUMN)));
			builder.addPropertyValue("shareUserColumn", DefinitionParserUtil.nullIfEmpty(element.getAttribute(SharePreferencesBeanDefinitionParser.SHARE_PREF_USER_COLUMN)));
			builder.addPropertyValue("shareTargetUserColumn",
					DefinitionParserUtil.nullIfEmpty(element.getAttribute(SharePreferencesBeanDefinitionParser.SHARE_PREF_TARGET_USER_COLUMN)));
			builder.addPropertyValue("shareTypeColumn", DefinitionParserUtil.nullIfEmpty(element.getAttribute(SharePreferencesBeanDefinitionParser.SHARE_PREF_TYPE_COLUMN)));
			builder.addPropertyValue("shareContentColumn", DefinitionParserUtil.nullIfEmpty(element.getAttribute(SharePreferencesBeanDefinitionParser.SHARE_PREF_CONTENT_COLUMN)));
			builder.addPropertyValue("shareMessageColumn", DefinitionParserUtil.nullIfEmpty(element.getAttribute(SharePreferencesBeanDefinitionParser.SHARE_PREF_MESSAGE_COLUMN)));
			builder.addPropertyValue("shareNameColumn", DefinitionParserUtil.nullIfEmpty(element.getAttribute(SharePreferencesBeanDefinitionParser.SHARE_PREF_NAME_COLUMN)));
			builder.setLazyInit(true);
		}
	}
}
