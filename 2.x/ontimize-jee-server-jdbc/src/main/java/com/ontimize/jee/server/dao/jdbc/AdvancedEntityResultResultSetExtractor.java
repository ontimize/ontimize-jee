...
public class AdvancedEntityResultResultSetExtractor implements ResultSetExtractor<AdvancedEntityResult> {
	 ...
	 * @param list
	 */
	public AdvancedEntityResultResultSetExtractor(final SQLStatementHandler sqlHandler, final QueryTemplateInformation queryTemplateInformation, final List<?> attributes, final int recordNumber, final int offset) {
		super();
	 ...
	 * @param list
	 */
	public AdvancedEntityResultResultSetExtractor(SQLStatementHandler sqlHandler, QueryTemplateInformation queryTemplateInformation, final int recordNumber, final int offset) {
	...
	public AdvancedEntityResult extractData(ResultSet resultSet) throws SQLException, DataAccessException {
		AdvancedEntityResult entityResult = new AdvancedEntityResult(EntityResult.OPERATION_SUCCESSFUL, EntityResult.DATA_RESULT);
		...
		try {
			if (this.sqlHandler.isPageable()) {
				...
				this.sqlHandler.resultSetToEntityResult(resultSet, entityResult, this.recordNumber, this.offset, this.sqlHandler.isDelimited(), this.attributes);
			}
