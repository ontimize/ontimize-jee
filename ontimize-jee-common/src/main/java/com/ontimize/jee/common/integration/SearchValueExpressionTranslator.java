...
public final class SearchValueExpressionTranslator {
	 ...
	 * @param expression
	 *            the expression
	 * @param entityEnumTranslator
	 *            the entity enum translator
	 * @return the search value expression
	 */
	public static <T extends EntityEnum, S extends EntityEnum> SearchValueExpression<T> translateExpression(final SearchValueExpression<S> expression,
		...
		for (final ISearchValue<S> searchValue : expression.getSearchValues()) {
			if (searchValue instanceof SearchValue) {
				searchValues.add(SearchValueExpressionTranslator.translateSearchValue((SearchValue<S>) searchValue, entityEnumTranslator));
				...
				searchValues.add(SearchValueExpressionTranslator.translateExpression(oldSearchValueExpression, entityEnumTranslator));
			} else {
	 ...
	 */
	public static <T extends EntityEnum, S extends EntityEnum> SearchValue<T> translateSearchValue(final SearchValue<S> searchValue,
	        final EntityEnumTranslator<S, T> entityEnumTranslator) {
		final SearchValue<T> newSearchValue = new SearchValue<>(entityEnumTranslator.translate(searchValue.getColumn()), searchValue.getQueryOperator(), searchValue.getValue());
