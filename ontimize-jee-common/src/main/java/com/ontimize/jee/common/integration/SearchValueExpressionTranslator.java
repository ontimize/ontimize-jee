...
public final class SearchValueExpressionTranslator {
    ...
    public static <T extends EntityEnum, S extends EntityEnum> SearchValueExpression<T> translateExpression(
            final SearchValueExpression<S> expression, final EntityEnumTranslator<S, T> entityEnumTranslator) {
        if (expression == null) {
            return null;
        }
        ...
        for (final ISearchValue<S> searchValue : expression.getSearchValues()) {
            if (searchValue instanceof SearchValue) {
                ...
                SearchValueExpressionTranslator.logger.error(
    ...
    public static <T extends EntityEnum, S extends EntityEnum> SearchValue<T> translateSearchValue(
            final SearchValue<S> searchValue, final EntityEnumTranslator<S, T> entityEnumTranslator) {
        final SearchValue<T> newSearchValue = new SearchValue<T>(
                entityEnumTranslator.translate(searchValue.getColumn()),
