...
public final class BasicExpressionTools {
	...
	public static boolean areEquals(BasicExpression newExpr, BasicExpression oldExpr) {
		if ((newExpr == null) == (oldExpr != null)) {
			return false;
		}
		if (newExpr == null) {
			return true;
		}
		Object[] newOperands = { newExpr.getLeftOperand(), newExpr.getRightOperand() };
		Object[] oldOperands = { oldExpr.getLeftOperand(), oldExpr.getRightOperand() };
		...
		for (int i = 0; i < newOperands.length; i++) {
			...
			if (nop instanceof BasicExpression) {
				...
				return ObjectTools.safeIsEquals(((BasicField) nop).toString(), ((BasicField) oop).toString());
			} else if (!ObjectTools.safeIsEquals(nop, oop)) {
				return false;
			}
		}
		return true;
	...
	public static BasicExpression completeExpresionFromKeys(BasicExpression sourceExpr, Map<Object, Object> kv,
		...
		for (Object obj : otherKeys) {
			if (obj instanceof String) {
				...
				sourceExpr = completeExpressionFromKeysIfBetweenDateFilter(sourceExpr, kv, obj);
			}
		}
		return sourceExpr;
	}

	private static BasicExpression completeExpressionFromKeysIfBetweenDateFilter(BasicExpression sourceExpr,
			Map<Object, Object> kv, Object obj) {
		...
		if ((fromDate != null) && (toDate != null)) {
			fieldExpr = BasicExpressionTools.convertSearchValue(basicField,
					new SearchValue(SearchValue.BETWEEN,
							new ArrayList<>(Arrays.asList(fromDate, toDate))));
	...
	private static void findConditionForFields(BasicExpression expr, List<BasicExpression> validExpr,
		...
		Object[] toCheck = { leftOperand, rightOperand };
		for (Object check : toCheck) {
			...
			} else if ((check instanceof BasicField) && fields.contains(((BasicField) check).toString())) {
				validExpr.add(expr);
			}
		}
	}
	...
	public static BasicExpression copyWithoutFields(BasicExpression expr, String... fieldNames) {
		...
		} else if ((leftOperand instanceof BasicField) && fields.contains(((BasicField) leftOperand).toString())) {
			newLeft = null;
		} else {
			newLeft = leftOperand;
		}
		...
		} else if ((rightOperand instanceof BasicField) && fields.contains(((BasicField) rightOperand).toString())) {
			newRight = null;
		} else {
		...
		if (newLeft instanceof BasicExpression) {
			return (BasicExpression) newLeft;
		}
		if (newRight instanceof BasicExpression) {
	...
	public static void renameField(BasicExpression expr, String fieldName, String newFieldName) {
		...
		for (Object check : toCheck) {
			...
			} else if ((check instanceof BasicField) && fieldName.equals(((BasicField) check).toString())) {
