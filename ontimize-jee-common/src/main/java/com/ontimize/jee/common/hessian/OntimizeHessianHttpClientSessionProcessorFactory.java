...
public final class OntimizeHessianHttpClientSessionProcessorFactory {
    ...
    private static HttpProcessor httpproc = HttpProcessorBuilder.create()
        ...
        .add(new RequestTargetAuthentication())
        .add(new RequestProxyAuthentication())
        .build();
    ...
    public static void addCredentials(URI uri, String userName, String password) {
        ...
        if (OntimizeHessianHttpClientSessionProcessorFactory.ENCRYPT) {
            OntimizeHessianHttpClientSessionProcessorFactory.credentials.put(
                    new AuthScope(host, port, AuthScope.ANY_REALM, AuthSchemes.BASIC),
                    new UsernamePasswordCredentials(userName, password));
            OntimizeHessianHttpClientSessionProcessorFactory.credentials.put(
                    new AuthScope(host, port, AuthScope.ANY_REALM, AuthSchemes.DIGEST),
                    new UsernamePasswordCredentials(userName,
                            OntimizeHessianHttpClientSessionProcessorFactory.encrypt(password)));
        } else {
    ...
    public static boolean removeCredentials(URI uri) {
        String host = uri.getHost();
        Integer port = uri.getPort();
        Credentials remove = OntimizeHessianHttpClientSessionProcessorFactory.credentials
    ...
    public static class SessionIdHttpRequestInterceptor implements HttpRequestInterceptor {
        ...
        public void process(HttpRequest request, HttpContext context) throws HttpException, IOException {
            if (!OntimizeHessianHttpClientSessionProcessorFactory.credentials.isEmpty()) {
                ...
                for (Entry<AuthScope, Credentials> entry : OntimizeHessianHttpClientSessionProcessorFactory.credentials
                    .entrySet()) {
