...
public abstract class AbstractGenericCache<K, V> {
	 ...
	 */
	protected AbstractGenericCache() {
	 ...
	 * @param ttl the ttl
	 */
	protected AbstractGenericCache(final long ttl) {
		this.ttl = ttl;
	...
	public V get(final K key) {
		...
		try {
			...
			if (cachedItem == null) {
				...
				AbstractGenericCache.logger.debug("Cache cargada para clave: {}. Valor: {}", key, result);
			} else {
				AbstractGenericCache.logger
				.debug("Acierto de cache para la clave: {}. Valor: {}", key, cachedItem.getValue());
			}
