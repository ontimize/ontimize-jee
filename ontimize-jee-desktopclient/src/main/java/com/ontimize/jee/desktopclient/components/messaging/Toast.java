...
/**
 * Muestra información al usuario al estilo del Toast de android. Se muestra una ventana que no bloquea la interacción del usuario y desaparece automáticamente pasado un tiempo.
 */
public class Toast implements Runnable, ActionListener {
	...
	protected JDialog createCurrentDialog(JComponent content, ModalityType modalityType, Rectangle bounds, Window parent) {
		...
		AWTUtilities.setWindowShape(dialog, new RoundRectangle2D.Float(0, 0, dialog.getWidth(), dialog.getHeight(), 5, 5));
	...
	protected void disposeDialog(JDialog currentDialog) {
		Component oldPanel = ((BorderLayout) currentDialog.getContentPane().getLayout()).getLayoutComponent(BorderLayout.CENTER);
		if (oldPanel != null) {
	 ...
	 *
	 * @param visible
	 *            the new toast visible with animation
	 */
