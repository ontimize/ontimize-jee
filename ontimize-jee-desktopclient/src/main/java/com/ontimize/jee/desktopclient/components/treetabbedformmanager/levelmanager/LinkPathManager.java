...
public class LinkPathManager implements PathManager {
	...
	public void showPath(List<Level> pathLevels) {
		...
		for (Level level : LinkPathManager.this.pathLevels) {
			if (i == (this.pathLevels.size() - 1)) {
				LinkPathManager.addLevel(this.column, LinkPathManager.this.rows, level, LinkPathManager.this, this.separator, true, this.resourceBundle);
			} else {
				LinkPathManager.addLevel(this.column, LinkPathManager.this.rows, level, LinkPathManager.this, this.separator, false, this.resourceBundle);
			}
			i++;
		}
	...
	/**
	 * An extension of JLabel which looks like a link and responds appropriately when clicked. Note that this class will only work with Swing 1.1.1 and later. Note that because of
	 * the way this class is implemented, getText() will not return correct values, user <code>getNormalText</code> instead.
	 */
