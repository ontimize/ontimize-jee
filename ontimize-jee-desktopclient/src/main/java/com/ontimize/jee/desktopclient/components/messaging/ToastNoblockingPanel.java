...
public class ToastNoblockingPanel extends AbstractToastPanel {
	...
	protected void init() {
		...
		this.scrollDsc = new AdaptableScrollPane(this.dscLabel, 1 - ToastNoblockingPanel.TITLE_VERSUS_DESCTIPTION_PERCENT);

		this.setLayout(new GridBagLayout());
		this.add(this.icoLabel, new GridBagConstraints(0, 0, 1, 2, 0, 1, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 3, 3, 10), 0, 0));
	...
	protected void setMessageLayout(ToastMessage msg, boolean useScroll) {
		this.bothMessages = !StringTools.isEmpty(msg.getTranslatedMessage(null)) && !StringTools.isEmpty(msg.getTranslatedDescription(null));
		if (useScroll) {
			...
			this.add(this.msgLabel, new GridBagConstraints(1, 0, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(3, 0, 2, 3), 0, 0));
			this.add(this.dscLabel, new GridBagConstraints(1, 1, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(2, 0, 3, 0), 0, 0));
		}
	}
	...
	protected void setMessageLabel(ToastMessage msg) {
		...
		this.msgLabel.setContentType((msgText != null) && msgText.toLowerCase().startsWith("<html>") ? "text/html" : "text");
		if ((msgText == null) || "text/html".equals(this.msgLabel.getContentType())) {
			this.msgLabel.setText(msgText);
		} else {
			String splittedText = this.considerToSplitTextLongerThanScreen(msgText, this.msgLabel.getFont(), this.msgLabel.getContentType());
	...
	protected void setMessageDesc(ToastMessage msg, boolean useScroll) {
		...
		this.dscLabel.setContentType((descText != null) && descText.toLowerCase().startsWith("<html>") ? "text/html" : "text");
		if ((descText == null) || "text/html".equals(this.dscLabel.getContentType())) {
			this.dscLabel.setText(descText);
		} else {
			String splittedText = this.considerToSplitTextLongerThanScreen(descText, this.dscLabel.getFont(), this.dscLabel.getContentType());
	...
	/*
	 * (non-Javadoc)
	 * @see com.utilmize.client.gui.toast.AbstractToastPanel#getResponse(com.utilmize.client.gui.toast.UToastMessage)
	 */
	@Override
	...
	protected class AdaptableScrollPane extends JScrollPane {
		...
		public Dimension getPreferredSize() {
			...
			if (preferredSize.height > (ToastNoblockingPanel.this.getMaximumHeight() * (ToastNoblockingPanel.this.bothMessages ? this.heightPercent : 1.0))) {
				preferredSize.height = (int) (ToastNoblockingPanel.this.getMaximumHeight() * (ToastNoblockingPanel.this.bothMessages ? this.heightPercent : 1.0));
			}
