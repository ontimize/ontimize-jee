 ...
 */
public class XMLLevelManagerBuilder extends XMLInterpreter implements com.ontimize.jee.desktopclient.components.treetabbedformmanager.levelmanager.builder.LevelManagerBuilder {
     ...
     *            Default package where the gui classes are stored
     * @throws Exception
     */
    ...
    private void preprocessChildren(List<CustomNode> unprocessedChildNodes, List<CustomNodeWrapper> preProcessedNodes, CustomNodeWrapper parentPreprocessedNode, LevelManager levelManager) {
        for (int i = 0; i < unprocessedChildNodes.size(); i++) {
            if (unprocessedChildNodes.get(i).isTag() == true) {
                ...
                if ((parentPreprocessedNode != null) && (parentPreprocessedNode.getAttributes().get(Level.NEXT_LEVEL) == null)) {
                    parentPreprocessedNode.getAttributes().put(Level.NEXT_LEVEL, attributes.get(Level.ID));
                } else if ((parentPreprocessedNode != null) && (parentPreprocessedNode.getAttributes().get(Level.NEXT_LEVEL) != null) && !parentPreprocessedNode.getAttributes().get(Level.NEXT_LEVEL).equals(attributes.get(Level.ID))) {
                    // en este caso este nodo esta anidado dentro de otro, que tiene un next level referenciado que no es este nodo
    ...
    protected void processChildren(List<CustomNode> childNodes, LevelManager guiNode, ResourceBundle resourceBundle) {
        List<CustomNodeWrapper> preProcessedNodes = new ArrayList<CustomNodeWrapper>();
        this.preprocessChildren(childNodes, preProcessedNodes, null, guiNode);

        // TODO cambiar esto por la interfaz Level
        List<Level> linkedTables = new ArrayList<Level>();
        ...
        if (!attributeTable.containsKey("visiblecols")) {
            if (attributeTable.containsKey("attr")) {
                if (attributeTable.containsKey("hideattr")) {
                    ...
                    List<String> hideAttr = Arrays.asList(StringUtils.split((String) attributeTable.get("hideattr"), ';'));
