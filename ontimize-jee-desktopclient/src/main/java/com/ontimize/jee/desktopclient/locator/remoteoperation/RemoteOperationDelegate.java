...
public class RemoteOperationDelegate implements ICallbackEventListener {
	 ...
	 *            the listener
	 */
	public RemoteOperationDelegate(String operationId, String className, Map<String, Object> parameters, IRemoteOperationListener listener) {
		super();
	...
	public void onCallbackMessageReceived(CallbackWrapperMessage message) {
		if (this.operationId.equals(message.getSubtype())) {
			if (RemoteOperationStatuses.WEBSOCKET_MESSAGE_TYPE_STATUS.equals(message.getType())) {
				...
				if (response != null) {
					try {
						this.webSocketClientHandler.sendMessage(response.getFirst(), this.operationId, response.getSecond());
					} catch (Exception error) {
						RemoteOperationDelegate.logger.error(null, error);
					}
				}
	...
	public void cancel() throws OntimizeJEEException {
		this.webSocketClientHandler.sendMessage(RemoteOperationStatuses.WEBSOCKET_MESSAGE_TYPE_CANCEL, this.operationId, new RemoteOperationCancelMessage());
	}
