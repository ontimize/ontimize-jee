...
public class IMSqlManager extends BasicInteractionManager {
	...
	/*
	 * (non-Javadoc)
	 * @see com.utilmize.client.fim.UBasicFIM#registerInteractionManager(com.ontimize.gui.Form, com.ontimize.gui.FormManager)
	 */
	@Override
		...
		public void actionPerformed(ActionEvent e) {
			new Thread(new Runnable() {
				...
				public void run() {
					...
					try {
						List<String> availableDataSources = BeansFactory.getBean(IServerManagementService.class).getAvailableDataSources();
					...
					if (sql != null) {
						try {
							...
							if ((connectionName == null) || connectionName.trim().isEmpty()) {
								IMSqlManager.this.managedForm.message("Seleccione el data source", Form.WARNING_MESSAGE);
								return;
							}
							...
							if (res.getCode() == EntityResult.OPERATION_WRONG) {
								...
								IMSqlManager.this.managedForm.message("Error en la consulta. " + res.getMessage(), Form.ERROR_MESSAGE);
								return;
							}
							if (res.isEmpty()) {
				...
				public void run() {
					if ((IMSqlManager.this.queryField.getValue() != null) && !"".equals(IMSqlManager.this.queryField.getValue())) {
						...
						if (selectedEntry != null) {
							...
							selectedEntry = new SentenceSql((String) IMSqlManager.this.queryField.getValue(), IMSqlManager.this.user);
		...
		public void actionPerformed(ActionEvent e) {
			new Thread(new Runnable() {
				...
				public void run() {
					...
					for (int i = 0; i < 1000; i++) {
						String preference = application.getPreferences().getPreference(IMSqlManager.this.user, "SQLSentence" + i);
						if (preference != null) {
