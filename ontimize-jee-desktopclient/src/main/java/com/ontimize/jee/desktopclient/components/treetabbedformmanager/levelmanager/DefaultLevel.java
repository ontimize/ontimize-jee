...
public class DefaultLevel extends Table implements Level {
    ...
    private final Map<Object, Object> lastSelectedKeys = new HashMap<Object, Object>();
    ...
    public String getDisplayText() {
        if ((this.displayTextFormatPattern != null) && (this.getSelectedRowsNumber() == 1)) {
            Hashtable<Object, Object> tableKeys = new Hashtable<Object, Object>();
            ...
            for (int i = 0; i < this.parentkeys.size(); i++) {
                ...
                if ((v != null) && (v.size() == 1)) {
                    ...
                    if (DefaultLevel.LOGGER.isTraceEnabled()) {
                        MessageDialog.showErrorMessage(this.parentFrame, "DEBUG: DefaultLevel: Parentkey " + this.parentkeys.get(i) + " is null. It won't be included in the query. "
    ...
    public void init(Hashtable parameters) throws Exception {
        ...
        this.showInMainFormAvailable = ParseUtils.getBoolean((String) parameters.get(Level.SHOW_IN_MAIN_FORM_AVAILABLE), true);
    ...
    public void openFormInMainTab(int rowIndex) {
        ...
        if ((this.formName != null) && bPemission && (this.isGrouped() == false)) {
            ...
            DefaultLevel.LOGGER.debug("Form is NULL. Or 'form' tag was not especified, or the user has not permission to open the form to query, or the table is grouped.");
        }
    }
    ...
    protected void setInnerValue(Object value, boolean autoSizeColumns) {
        ...
        if (!this.lastSelectedKeys.isEmpty()) {
            int rowForKeys = this.getRowForKeys(new Hashtable<Object, Object>(this.lastSelectedKeys));
            if (rowForKeys != -1) {
