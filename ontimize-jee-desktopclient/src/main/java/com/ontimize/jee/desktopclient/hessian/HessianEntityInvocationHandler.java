...
/**
 * The Class HessianEntityInvocationHandler. Se encarga de realizar el mapeo nombre_entidad+método -> método del servicio Por ejemplo, cuando un
 * formulario quiera hacer un "query" a la entidad "masterDataService.user" esta clase redireccionará la llamada al método "userQuery" del servicio
 * "masterDataService".
 */
public class HessianEntityInvocationHandler extends AbstractInvocationDelegate implements AdvancedEntity, AdvancedQueryEntity, InvocationHandler {
    ...
    public HessianEntityInvocationHandler(String entityName) {
        ...
        /**
     * Devuelve el servicio asociado a la entidad. Por defecto se devuelve el servicio estándar de ontimize que se encarga en la parte servidor de
     * realizar la petición al repositorio de la entidad.
     *
    ...
    protected void checkHessianMethodExists(Method hessianMethod, String methodName) throws InvocationTargetException {
        if (hessianMethod == null) {
            throw new InvocationTargetException(null, "Method " + methodName + " not found in service" + this.serviceName);
        }
    }
    ...
    /*
     * (non-Javadoc)
     * @see com.ontimize.db.AdvancedEntity#query(java.util.Hashtable, java.util.Vector, int, int, int, java.lang.String, boolean)
     */
    @Override
    public AdvancedEntityResult query(Hashtable kv, Vector attributes, int sessionId, int recordNumber, int startIndex, String orderBy, boolean desc)
            throws Exception {
     ...
     */
    @Override
    public AdvancedEntityResult query(Hashtable kv, Vector attributes, int sessionId, int recordNumber, int startIndex, Vector orderBy)
            throws Exception {
        // TODO ojo que hessian no permite polimorfismo
        return null;
    }
