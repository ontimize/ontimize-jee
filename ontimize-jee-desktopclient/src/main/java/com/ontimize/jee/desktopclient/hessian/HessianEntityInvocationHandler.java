...
/**
 * The Class HessianEntityInvocationHandler. Se encarga de realizar el mapeo nombre_entidad+método -> método del servicio Por ejemplo, cuando un formulario quiera hacer un
 * "query" a la entidad "masterDataService.user" esta clase redireccionará la llamada al método "userQuery" del servicio "masterDataService".
 */
public class HessianEntityInvocationHandler extends AbstractInvocationDelegate implements AdvancedEntity, AdvancedQueryEntity, InvocationHandler {
	...
	/**
	 * Devuelve el servicio asociado a la entidad. Por defecto se devuelve el servicio estándar de ontimize que se encarga en la parte servidor de realizar la petición al
	 ...
	 */
	@Override
	public AdvancedEntityResult query(Hashtable kv, Vector attributes, int sessionId, int recordNumber, int startIndex, String orderBy, boolean desc) throws Exception {
