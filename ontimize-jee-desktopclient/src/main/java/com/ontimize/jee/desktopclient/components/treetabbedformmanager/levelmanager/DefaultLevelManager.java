...
public class DefaultLevelManager extends Column implements LevelManager {
    ...
    private final Vector<String> listLevels = new Vector<String>(1);
    ...
    public void init(Hashtable parameters) {
        ...
        String defaultButtons = ConcatTools.concat(DefaultLevelManager.QUERY_BUTTON, '|', DefaultLevelManager.PREVIOUS_BUTTON, '|', DefaultLevelManager.HOME_BUTTON, '|', DefaultLevelManager.REFRESH_BUTTON, '|', DefaultLevelManager.NEXT_BUTTON, '|', DefaultLevelManager.INSERT_BUTTON);
        ...
        String defaultSections = ConcatTools.concat(DefaultLevelManager.BUTTONS_SECTION, '|', DefaultLevelManager.PATH_SECTION, '|', DefaultLevelManager.MAIN_SECTION);
    ...
    protected Button createButton(String button) {
        ...
        if (DefaultLevelManager.QUERY_BUTTON.equalsIgnoreCase(button)) {
            this.bSearch = this.createButton(DefaultLevelManager.SEARCH_KEY, "search", DefaultLevelManager.SEARCH_ICON, new ActionListener() {

                @Override
                public void actionPerformed(ActionEvent e) {
                    if (DefaultLevelManager.this.currentShowLevel != null) {
                        Level level = DefaultLevelManager.this.getLevel(DefaultLevelManager.this.currentShowLevel);
                        if (level != null) {
            ...
            this.bPrevious = this.createButton(DefaultLevelManager.PREV_KEY, "previous", DefaultLevelManager.PREV_ICON, new ActionListener() {
            ...
            this.bHome = this.createButton(DefaultLevelManager.HOME_KEY, "home", DefaultLevelManager.HOME_ICON, new ActionListener() {
            ...
            this.bRefresh = this.createButton(DefaultLevelManager.REFRESH_KEY, "refresh", DefaultLevelManager.REFRESH_ICON, new ActionListener() {

                @Override
                public void actionPerformed(ActionEvent e) {
                    System.out.println("refresh actual table.");
            ...
            this.bNext = this.createButton(DefaultLevelManager.NEXT_KEY, "next", DefaultLevelManager.NEXT_ICON, new ActionListener() {

                @Override
                public void actionPerformed(ActionEvent e) {
                    String next = DefaultLevelManager.this.getNextPanel(DefaultLevelManager.this.currentShowLevel);
                    DefaultLevelManager.this.show(next);
                }
            });
            b = this.bNext;
        } else if (DefaultLevelManager.INSERT_BUTTON.equalsIgnoreCase(button)) {
            this.bNewForm = this.createButton(DefaultLevelManager.NEW_FORM_KEY, "newform", DefaultLevelManager.NEW_FORM_ICON, new ActionListener() {
    ...
    public void updateDisplayPath() {
        List<Level> paths = new ArrayList<Level>();
    ...
    /**
     * Creates the detail form for this table. The detail form is used to insert
     * new records to the table and to diplay de information contained buy the
     * table in a more detailed way using a form instead a row.
     *
     * @param iFormManager
     */
    ...
    public void openFormInMainTab(Level sourceLevel, int mode) {
        if (this.parentForm.getFormManager() instanceof ITreeTabbedFormManager) {
            IDetailForm detailForm = this.createTabbedDetailForm((ITreeTabbedFormManager) this.parentForm.getFormManager(), sourceLevel.getId());
