...
public class StatusInformationPanel extends JPanel {
	...
	private void init() {
		...
		this.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(StatusInformationPanel.borderColor, 1), BorderFactory.createEmptyBorder(20, 10, 0, 10)));
		...
		this.add(this.estimatedTime, new GridBagConstraints(1, 0, 2, 1, 1.0D, 0.0D, 13, 0, new Insets(6, 1, 1, 5), 0, 0));
		...
		this.add(this.cancelButton, new GridBagConstraints(0, 3, 2, 1, 0.0D, 0.0D, 10, 0, new Insets(1, 2, 5, 5), 0, 0));
		...
		public StatusInformationProgressBar() {
			...
			this.addHierarchyListener(new HierarchyListener() {
				...
				public void hierarchyChanged(HierarchyEvent e) {
					if (((e.getChangeFlags() & HierarchyEvent.SHOWING_CHANGED) > 0) && StatusInformationProgressBar.this.isShowing()) {
						if (StatusInformationProgressBar.this.customIndeterminate) {
							new Thread(StatusInformationProgressBar.this.indeterminateTask, "indeterminate thread").start();
						}
					}
				}
			});
