...
public final class BasicExpressionUtils {
		...
		switch (searchValue.getCondition()) {
			...
			case SearchValue.IN:
				if ((searchValue.getValue() instanceof List) && (((List) searchValue.getValue()).size() >= 1000)) {
					return BasicExpressionUtils.convertSearchValueInComplex(basicField, (List<Object>) searchValue.getValue());
				}
	...
	private static BasicExpression convertSearchValueInComplex(BasicField basicField, List<Object> value) {
		...
		for (int i = 0; i < value.size(); i += 999) {
          ...
          .combineExpressionOr(be, new BasicExpression(basicField, BasicOperator.IN_OP, value.subList(i, Math.min(i + 999, value.size()))));
		}
		return be;
	}
	...
	public static BasicExpression completeExpresionFromKeys(BasicExpression sourceExpr, Map<Object, Object> kv, List<Object> otherKeys) {
		...
		for (Object obj : otherKeys) {
			if (obj instanceof String) {
				...
				// Simple filter from two attrs in form (since - to) to single db column
				Object fromDate = BasicExpressionUtils.getDate(kv.get(((BetweenDateFilter) obj).getSinceFormFieldAttr()));
              ...
              .convertSearchValue(basicField, new SearchValue(SearchValue.MORE_EQUAL, fromDate));
					...
					fieldExpr = BasicExpressionUtils.convertSearchValue(basicField, new SearchValue(SearchValue.LESS_EQUAL, toDate));
				}
				if (fieldExpr != null) {
					sourceExpr = BasicExpressionUtils.combineExpression(sourceExpr, fieldExpr);
					...
					if (dataFieldValue != null) {
						...
						if (checked) {
							sourceExpr = BasicExpressionUtils.combineExpression(sourceExpr,
									BasicExpressionUtils.convertSearchValue(basicField, new SearchValue(SearchValue.NOT_NULL, null)));
						} else {
							sourceExpr = BasicExpressionUtils.combineExpression(sourceExpr,
									BasicExpressionUtils.convertSearchValue(basicField, new SearchValue(SearchValue.NULL, null)));
						}
					}
				}
			}
