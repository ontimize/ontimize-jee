 ...
 */
public final class BasicExpressionUtils {

    private static final Logger logger = LoggerFactory.getLogger(BasicExpressionUtils.class);
        ...
        for (int i = 0; i < newOperands.length; i++) {
            ...
            if (nop.getClass().equals(oop.getClass())) {
                if (nop instanceof BasicExpression) {
                    if (!BasicExpressionUtils.areEquals((BasicExpression) nop, (BasicExpression) oop)) {
                        return false;
                    }
    ...
    public static BasicExpression combineExpression(BasicExpression... exprs) {
        return BasicExpressionUtils.combineExpression(BasicOperator.AND_OP, exprs);
    }
    ...
    public static BasicExpression combineExpressionOr(BasicExpression... exprs) {
        return BasicExpressionUtils.combineExpression(BasicOperator.OR_OP, exprs);
    }
        ...
        switch (searchValue.getCondition()) {
            ...
            case SearchValue.IN:
                if ((searchValue.getValue() instanceof List) && (((List) searchValue.getValue()).size() >= 1000)) {
                    return BasicExpressionUtils.convertSearchValueInComplex(basicField,
            ...
            case SearchValue.BETWEEN:
                return BasicExpressionUtils.convertSearchValueBetween(basicField, searchValue);
            case SearchValue.NOT_BETWEEN:
                return BasicExpressionUtils.convertSearchValueNotBetween(basicField, searchValue);
    ...
    private static BasicExpression convertSearchValueInComplex(BasicField basicField, List<Object> value) {
        ...
        for (int i = 0; i < value.size(); i += 999) {
            be = BasicExpressionUtils
                .combineExpressionOr(be, new BasicExpression(basicField, BasicOperator.IN_OP,
    ...
    /**
     * Ensure to complete a BasicExpression with filters with another "susceptible" filters form another
     * keys, depending on values at form. For each "otherKey" received, if tis value is not null at form
     * will be considered to add as a filter.
     * @param sourceExpr the source expr
     * @param form the form
     * @param otherKeys List of possible keys to add to filter. It is accepted "String-with-attr",
     *        "SimpleFieldFilter-object" or "BetweenDateFilter-object"
     * @return the basic expression
     */
    public static BasicExpression completeExpresionFromKeys(BasicExpression sourceExpr, Form form,
        ...
        return BasicExpressionUtils.completeExpresionFromKeys(sourceExpr, kv, otherKeys);
    }
        ...
        for (Object obj : otherKeys) {
            if (obj instanceof String) {
                ...
                if (value != null) {
                    ...
                    BasicExpression fieldExpr = BasicExpressionUtils.composeExpressionFromValue(basicField, value);
                    sourceExpr = BasicExpressionUtils.combineExpression(sourceExpr, fieldExpr);
                }
            } else if (obj instanceof SimpleFieldFilter) {
                // Simple filter from attr in form to another named db column
                ...
                // Simple filter from two attrs in form (since - to) to single db column
                Object fromDate = BasicExpressionUtils
                    .getDate(kv.get(((BetweenDateFilter) obj).getSinceFormFieldAttr()));
                Object toDate = BasicExpressionUtils.getDate(kv.get(((BetweenDateFilter) obj).getToFormFieldAttr()));
                ...
                if (fieldExpr != null) {
                    ...
                    if (dataFieldValue != null) {
                        ...
                        try {
                            ...
                            BasicExpressionUtils.logger.debug("error getting boolean value, parsing...", ex);
                            checked = ParseTools.getBoolean(String.valueOf(dataFieldValue), false);
                        }
                        if (checked) {
                            sourceExpr = BasicExpressionUtils.combineExpression(sourceExpr,
                                    BasicExpressionUtils.convertSearchValue(basicField,
    ...
    public static BasicExpression composeExpressionFromValue(BasicField basicField, Object value) {
        ...
        if (value == null) {
            ...
            fieldExpr = BasicExpressionUtils.convertSearchValue(basicField, (SearchValue) value);
    ...
    public static BasicExpression findConditionForFields(BasicExpression expr, String... fieldNames) {
        ...
        BasicExpressionUtils.findConditionForFields(expr, validExpr, fields);
        return BasicExpressionUtils.combineExpression(validExpr.toArray(new BasicExpression[] {}));
    }
    ...
    private static void findConditionForFields(BasicExpression expr, List<BasicExpression> validExpr,
        ...
        for (Object check : toCheck) {
            if (check instanceof BasicExpression) {
                BasicExpressionUtils.findConditionForFields((BasicExpression) check, validExpr, fields);
    ...
    public static BasicExpression copyWithoutFields(BasicExpression expr, String... fieldNames) {
        ...
        if (leftOperand instanceof BasicExpression) {
            newLeft = BasicExpressionUtils.copyWithoutFields((BasicExpression) leftOperand, fieldNames);
        ...
        if (rightOperand instanceof BasicExpression) {
            newRight = BasicExpressionUtils.copyWithoutFields((BasicExpression) rightOperand, fieldNames);
    ...
    public static BasicExpression replaceExpression(BasicExpression originalExpr, BasicExpression ipExpr,
        ...
        Object newLeft = null;
        if (leftOperand instanceof BasicExpression) {
            newLeft = BasicExpressionUtils.replaceExpression((BasicExpression) leftOperand, ipExpr, replaceExpr);
        } else {
            newLeft = leftOperand;
        }
        ...
        Object newRight = null;
        if (rightOperand instanceof BasicExpression) {
            newRight = BasicExpressionUtils.replaceExpression((BasicExpression) rightOperand, ipExpr, replaceExpr);
        } else {
            newRight = rightOperand;
        }
        if ((newRight != null) && (newLeft != null)) {
    ...
    public static void renameField(BasicExpression expr, String fieldName, String newFieldName) {
        ...
        for (Object check : toCheck) {
            if (check instanceof BasicExpression) {
                BasicExpressionUtils.renameField((BasicExpression) check, fieldName, newFieldName);
            } else if (check instanceof BasicField) {
    ...
    public static boolean containsField(BasicExpression expr, String fieldName) {
        ...
        for (Object check : toCheck) {
            if (check instanceof BasicExpression) {
                if (BasicExpressionUtils.containsField((BasicExpression) check, fieldName)) {
                    return true;
                }
            } else if (check instanceof BasicField) {
                if (fieldName.equals(((BasicField) check).toString())) {
    ...
    public static String toString(BasicExpression be) {
        ...
        BasicExpressionUtils.toString(be, sb);
        return sb.toString();
    }
    ...
    private static void toString(BasicExpression be, StringBuilder sb) {
        ...
        if (left instanceof BasicExpression) {
            BasicExpressionUtils.toString((BasicExpression) left, sb);
        } else {
        ...
        if (right != null) {
            if (right instanceof BasicExpression) {
                BasicExpressionUtils.toString((BasicExpression) right, sb);
            } else {
