...
final class DigestAuthUtils {
    ...
    /**
     * Computes the <code>response</code> portion of a Digest authentication header. Both the server and
     * user agent should compute the <code>response</code> independently. Provided as a static method to
    ...
    static String generateDigest(boolean passwordAlreadyEncoded, String username, String realm, String password,
            String httpMethod, String uri, String qop, String nonce, String nc,
     ...
     * <code>removeCharacter</code>, and splits the element based on the <code>delimiter</code>. A
     * <code>Map</code> is then generated, with the left of the delimiter providing the key, and the
     ...
     * @param removeCharacters one or more characters to remove from each element prior to attempting
     *        the split operation (typically the quotation mark symbol) or <code>null</code> if no
     *        removal should occur
     * @return a <code>Map</code> representing the array contents, or <code>null</code> if the array to
     ...
     *         delimiter (neither element includes the delimiter)
