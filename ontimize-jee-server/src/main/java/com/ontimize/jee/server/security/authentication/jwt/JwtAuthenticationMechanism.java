...
public class JwtAuthenticationMechanism implements IAuthenticationMechanism {
    ...
    public AuthenticationResult authenticate(HttpServletRequest request, HttpServletResponse response,
        ...
        return new AuthenticationResult(this.refreshToken,
                this.composeAthenticationToken(authToken, userDetailsService));
    }

    protected Authentication composeAthenticationToken(String token, UserDetailsService userDetailsService)
        ...
        if ((this.tokenExpirationTime > 0)
                && ((tokenCreationTime + this.tokenExpirationTime) < System.currentTimeMillis())) {
            JwtAuthenticationMechanism.logger.debug("token expired {} : {}", this.tokenExpirationTime,
                    tokenCreationTime);
