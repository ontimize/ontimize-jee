...
public class TableAttributeDispatcher extends AbstractAttributeDispatcher<TableAttribute> {
    ...
    /** The logger. */
    private final static Logger	logger	= LoggerFactory.getLogger(TableAttributeDispatcher.class);

    /*
     * (non-Javadoc)
     * @see com.ontimize.jee.server.services.core.IAttributeDispatcher#processAttribute(java.lang.Object, com.ontimize.db.EntityResult,
     * org.springframework.context.ApplicationContext)
     */
    @Override
    public void processQueryAttribute(TableAttribute tableAttribute, EntityResult result, ApplicationContext applicationContext) {
        String entityName = tableAttribute.getEntity();
        if ((entityName == null) || (tableAttribute.getRecordNumberToInitiallyDownload() == 0)) {
            TableAttributeDispatcher.logger.warn(" INVALID TABLEATTRIBUTE - WITHOUT \"ENTITY\" NOR FOR \"RECORDNUMBERTOINITIALLYDOWNLOAD\" LENGTH ZERO. CHECK CONFIGURATION.");
            return;
        }
    ...
    protected EntityResult processQueryAttribute(TableAttribute tableAttribute, EntityResult result, ApplicationContext appContext, int k) {
        ...
        // if ((entity instanceof AdvancedEntity == false) && requestedRecordNumber > 0) {
    ...
    private Map<Object, Object> collectKeysValues(TableAttribute tableAttribute, EntityResult result, int k) {
        ...
        for (int j = 0; j < parentkeys.size(); j++) {
            ...
            if (vKeyValues == null) {
                TableAttributeDispatcher.logger.warn(" RESULT VECTOR NOT FOUND IN QUERY RESULT NEITHER FOR PARENTKEY: '{}' NOR FOR: '{}'. CHECK CONFIGURATION.", parentkeys.get(j), parentkeyColumn);
            }
            Object oOtherKeyValue = vKeyValues == null ? null : vKeyValues.get(k);
            TableAttributeDispatcher.logger.trace("The entity \"{}\" receives the value \"{}\" for the column with alias \"{}\"", tableAttribute.getEntity(), oOtherKeyValue, parentkeyColumn);
            ...
            if (oOtherKeyValue != null) {
				...
				// finalKeysValues.put(tableAttribute.getParentkeyEquivalence(parentkeys.get(j).toString()), new NullValue(result.getColumnSQLType(parentkeys.get(j).toString())));
            }
        }

        // Consider QueryFilter --------------------------------
        if (tableAttribute.getQueryFilter() != null) {
            TableAttributeDispatcher.logger.trace("The entity \"{}\" receives moreover this queryFilters \"{}\".", tableAttribute.getEntity(), tableAttribute.getQueryFilter());
    ...
    /*
     * (non-Javadoc)
     * @see com.ontimize.jee.server.services.core.IAttributeDispatcher#processInsertAttribute(java.lang.Object,
    ...
    /*
     * (non-Javadoc)
     * @see com.ontimize.jee.server.services.core.IAttributeDispatcher#processUpdateAttribute(java.lang.Object,
     ...
     */
    @Override
    public EntityResult processUpdateAttribute(TableAttribute attribute, UpdateOperation updateOperation, Map<?, ?> generatedValuesInParentEntity,
            Map<?, ?> valuesInParentEntity, ApplicationContext applicationContext) {
        String entityName = attribute.getEntity();
        if (entityName != null) {
    ...
    /*
     * (non-Javadoc)
     * @see com.ontimize.jee.server.services.core.IAttributeDispatcher#processDeleteAttribute(java.lang.Object,
     ...
     */
    @Override
    public EntityResult processDeleteAttribute(TableAttribute attribute, DeleteOperation deleteOperation, Map<?, ?> generatedValuesInParentEntity,
