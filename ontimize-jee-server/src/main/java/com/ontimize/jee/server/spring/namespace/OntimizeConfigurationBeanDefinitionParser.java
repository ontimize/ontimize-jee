...
public class OntimizeConfigurationBeanDefinitionParser extends AbstractSingleBeanDefinitionParser {
    ...
    protected void doParse(Element element, ParserContext ctx, BeanDefinitionBuilder builder) {
        ...
        Element remoteOperation = DomUtils.getChildElementByTagName(element,
                OntimizeConfigurationBeanDefinitionParser.REMOTE_OPERATION_ATTR);
        ...
        Element security = DomUtils.getChildElementByTagName(element,
                OntimizeConfigurationBeanDefinitionParser.SECURITY_ATTR);
        if (security != null) {
            // Just make a new Parser for each one and let the parser do the work
        ...
        Element remotePreferences = DomUtils.getChildElementByTagName(element,
                OntimizeConfigurationBeanDefinitionParser.REMOTE_PREFERENCES_ATTR);
        ...
        // Support for i18n
        Element remoteI18n = DomUtils.getChildElementByTagName(element,
                OntimizeConfigurationBeanDefinitionParser.REMOTE_I18N);
        if (remoteI18n != null) {
            // Just make a new Parser for each one and let the parser do the work
        ...
        Element formProvider = DomUtils.getChildElementByTagName(element,
                OntimizeConfigurationBeanDefinitionParser.REMOTE_FORM_PROVIDER);
        ...
        Element sharePreferences = DomUtils.getChildElementByTagName(element,
                OntimizeConfigurationBeanDefinitionParser.REMOTE_SHARE_PREFERENCES);
        ...
        // Support for mail
        Element mail = DomUtils.getChildElementByTagName(element,
                OntimizeConfigurationBeanDefinitionParser.REMOTE_MAIL);
        if (mail != null) {
            // Just make a new Parser for each one and let the parser do the work
