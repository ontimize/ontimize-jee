...
public class Log4j2LoggerHelper implements ILoggerHelper {
	...
	public EntityResult getLogFiles() throws Exception {
		...
		Files.walkFileTree(folder, new java.nio.file.SimpleFileVisitor<Path>() {
			...
			public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
				res.addRecord(EntityResultTools.keysvalues("FILE_NAME", file.toString(), "FILE_SIZE", Files.size(file)));
	...
	// For some extrange reason, when a lloger is requested to logerFactory it gets from a "Default" context, and not from our own context.
