...
public class CorsBeanDefinitionParser implements BeanDefinitionParser {
    ...
    private static final List<String> DEFAULT_ALLOWED_METHODS = Arrays.asList(HttpMethod.GET.name(),
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) {
        ...
        CorsBeanDefinitionParser.registerCorsConfigurations(corsConfigurations, parserContext,
     ...
     * well-known name unless already registered. The bean definition may be updated if a non-null CORS
    ...
    public static RuntimeBeanReference registerCorsConfigurations(Map<String, CorsConfiguration> corsConfigurations,
        ...
        if (!parserContext.getRegistry()
            .containsBeanDefinition(CorsBeanDefinitionParser.CORS_CONFIGURATION_BEAN_NAME)) {
            ...
            parserContext.getReaderContext()
                .getRegistry()
                .registerBeanDefinition(CorsBeanDefinitionParser.CORS_CONFIGURATION_BEAN_NAME, corsConfigurationsDef);
            parserContext.registerComponent(new BeanComponentDefinition(corsConfigurationsDef,
                    CorsBeanDefinitionParser.CORS_CONFIGURATION_BEAN_NAME));
        } else if (corsConfigurations != null) {
            BeanDefinition corsConfigurationsDef = parserContext.getRegistry()
                .getBeanDefinition(CorsBeanDefinitionParser.CORS_CONFIGURATION_BEAN_NAME);
            corsConfigurationsDef.getConstructorArgumentValues().addIndexedArgumentValue(0, corsConfigurations);
        }
