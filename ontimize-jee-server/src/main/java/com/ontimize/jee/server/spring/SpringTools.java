...
public final class SpringTools {
    ...
    public static Map<String, Object> getAllEnvironmentProperties(EnvironmentCapable context) {
        ...
        for (Iterator<?> it = ((ConfigurableEnvironment) context.getEnvironment()).getPropertySources().iterator(); it
            .hasNext();) {
            ...
            if (propertySource instanceof EnumerablePropertySource) {
                ...
                for (String key : ((EnumerablePropertySource) propertySource).getPropertyNames()) {
                    try {
                        ...
                        map.put(key, value);
                    } catch (IllegalArgumentException error) { // This key includes an unresolveable placeholder, and
                                                               // will not be part of the map that we output
                        // do nothing
