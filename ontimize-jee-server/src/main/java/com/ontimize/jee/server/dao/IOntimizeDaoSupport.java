...
public interface IOntimizeDaoSupport {
    ...
    /**
     * This method must implement a standard pageable query operation returning the set of data that
     * matches the conditions specified by the <code>keysValues</code> parameter. The
     ...
     * @param attributes a list of columns or attributes that must be recovered for each record
     *        returned. Cannot be null. If empty, all attributes should be returned.
     * @param recordNumber number of records will be recovered.
     * @param startIndex index of the row where the query starts.
     * @param orderBy the sort
     * @param queryId the query id
     * @return a EntityResult with the resulting set of data. This result can be empty if no results
     *         exist, and if an error has ocurred this will be indicated in the result.
     */
    AdvancedEntityResult paginationQuery(Map<?, ?> keysValues, List<?> attributes, int recordNumber, int startIndex,
            List<?> orderBy, String queryId);

    AdvancedEntityResult paginationQuery(Map<?, ?> keysValues, List<?> attributes, int recordNumber, int startIndex,
            List<?> orderBy, String queryId, ISQLQueryAdapter adapter);
