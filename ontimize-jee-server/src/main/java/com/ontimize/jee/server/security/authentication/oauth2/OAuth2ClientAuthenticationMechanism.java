...
public class OAuth2ClientAuthenticationMechanism implements IAuthenticationMechanism, InitializingBean {
	...
	protected AuthenticationResult authenticate(String token, OntimizeWebAuthenticationDetails authenticationDetails) {
		...
		Map<String, Object> userInfo = this.oAuth2ClientUserInfoProvider.getUserInfoFromProvider(this.getAccessToken(token, authenticationDetails), this.oAuth2ClientProperties);
		...
		return new AuthenticationResult(true, new UsernamePasswordAuthenticationToken(idAsString, OntimizeAuthenticationProvider.NO_AUTHENTICATION_TOKEN, null));
	}
	...
	protected String getAccessToken(String codeToken, OntimizeWebAuthenticationDetails authenticationDetails) {
		...
		try {
			...
			OAuth2ClientAuthenticationMechanism.log.error("Error thrown by RestTemplate client when exchanging code for token", e);
	...
	private Map<String, Object> getUserDataMapFrom(String codeToken, OntimizeWebAuthenticationDetails authenticationDetails) {
		...
		ResponseEntity<Map> userInfoMapEntity = restTemplate.exchange(this.oAuth2ClientProperties.getAccessTokenUri(), HttpMethod.POST, entity, Map.class);
