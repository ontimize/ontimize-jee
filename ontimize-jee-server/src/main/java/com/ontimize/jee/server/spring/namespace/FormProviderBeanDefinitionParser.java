...
public class FormProviderBeanDefinitionParser extends AbstractSingleBeanDefinitionParser {
     ...
     * @param element {@link Element} The tag element
     * @param ctx {@link ParserContext} The context in which the parsing is occuring
     * @param builder {@link BeanDefinitionBuilder} The bean definitions build to use.
     */
    @Override
    protected void doParse(Element element, ParserContext parserContext, BeanDefinitionBuilder builder) {
        Element item = DomUtils.getChildElementByTagName(element,
                FormProviderBeanDefinitionParser.FORM_PROVIDER_ENGINE_PROPERTY);
        Element child = DomUtils.getChildElements(item).get(0);
        Object engine = null;

        if (child.getLocalName()
            .equals(FormProviderBeanDefinitionParser.FORM_PROVIDER_DATABASE_CONFIGURATION_PROPERTY)) {
            final ParserContext nestedCtx = new ParserContext(parserContext.getReaderContext(),
                    parserContext.getDelegate(), builder.getBeanDefinition());
            engine = new FormProviderParser().parse(child, nestedCtx);
        } else {
            engine = DefinitionParserUtil.parseNode(child, parserContext, builder.getBeanDefinition(),
                    element.getAttribute(FormProviderBeanDefinitionParser.SCOPE), false);
        }
    ...
    public static class FormProviderParser extends AbstractSingleBeanDefinitionParser {
        ...
        @Override
        protected void doParse(Element element, ParserContext parserContext, BeanDefinitionBuilder builder) {
            builder.addPropertyReference("daoFormProvider", DefinitionParserUtil
                .nullIfEmpty(element.getAttribute(FormProviderBeanDefinitionParser.FORM_PROVIDER_REF_REPOSITORY)));

            builder.addPropertyValue("formProviderIdColumn", DefinitionParserUtil
                .nullIfEmpty(element.getAttribute(FormProviderBeanDefinitionParser.FORM_PROVIDER_ID_COLUMN)));
            builder.addPropertyValue("formProviderFormNameColumn",
                    DefinitionParserUtil.nullIfEmpty(
                            element.getAttribute(FormProviderBeanDefinitionParser.FORM_PROVIDER_FORM_NAME_COLUMN)));
            builder.addPropertyValue("formProviderFormXMLColumn",
                    DefinitionParserUtil.nullIfEmpty(
                            element.getAttribute(FormProviderBeanDefinitionParser.FORM_PROVIDER_FORM_XML_COLUMN)));
            builder.setLazyInit(true);
        }
