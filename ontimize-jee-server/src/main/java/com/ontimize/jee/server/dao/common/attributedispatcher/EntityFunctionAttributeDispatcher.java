...
public class EntityFunctionAttributeDispatcher extends AbstractAttributeDispatcher<EntityFunctionAttribute> {
	...
	@Override
	public void processQueryAttribute(EntityFunctionAttribute attributeAdv, EntityResult result, ApplicationContext applicationContext) {
		...
		if (entityName != null) {
			...
			for (int k = 0; k < recordCount; k++) {
				...
				for (int m = 0; m < filterAttributes.size(); m++) {
					...
					if ((keyValues != null) && (keyValues instanceof Vector) && (((Vector<?>) keyValues).get(k) != null)) {
